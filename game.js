...
import {prepareDay18V4GameEntry, getDay18V4GameSegment, getDay18V4GameResumePresentation, applyDay18V4GameChoice, completeDay18V4GameChapter} from "./src/day18-v4-game-bridge.mjs?v=62";
...
function openStoryScene(scene) {
  ...
  const freeResume=isStoryFreeActionResume(state,scene.id)&&!(lockedDay23&&day23V4)&&!(lockedDay24&&day24V4);
...
function skipImmersiveScene(event) {
  ...
  const resolution=immersiveScene.sequence.find(step=>step.type==="haeunContactResolution"||step.type==="haeunStayResolution"||step.type==="contactConsentCue"||step.type==="lodgingConsentCue"||step.type==="travelConfirmationCue"||step.type==="photoConsentCue"||isDay23V4ResolutionStep(step)||isDay24V4ResolutionStep(step));
  ...
  if(completionCue){
    try{
      if(completionCue.day===24)completeDay24V4GameChapter(state,completionCue);
...
function finishImmersiveScene() {
  ...
  if(completedSession?.id===LOCKED_DAY25_SCENE_ID&&state.storyFlags?.day25GuestStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY25_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY25_SCENE_ID,getLockedDay25LegacyChoice(state));state.storyFlags.day25RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY25_SCENE_ID&&state.storyFlags?.day25RuntimeComplete&&!state.storyFlags?.day25FreeActionComplete){const freeAction={type:"freeAction",id:"day25-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
