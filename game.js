...
function openStoryScene(scene) {
  ...
  const freeResume=isStoryFreeActionResume(state,scene.id)&&!(lockedDay23&&day23V4)&&!(lockedDay24&&day24V4)&&!(lockedDay25&&day25V4)&&!(lockedDay26&&day26V4)&&!(lockedDay27&&day27V4)&&!(lockedDay28&&day28V4)&&!(lockedDay29&&day29V4);
  ...
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:freeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:state.storyFreeAction.phase,location:state.storyFreeAction.location,maxActions:state.storyFreeAction.maxActions},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?(day6V3?getDay6V3ImmersiveSegment(state).steps:getLockedDay6Segment(state)):lockedDay7?(day7V3?getDay7V3ImmersiveSegment(state).steps:getLockedDay7Segment(state)):lockedDay8?(day8V3?getDay8V3ImmersiveSegment(state).steps:getLockedDay8Segment(state)):lockedDay9?(day9V3?getDay9V3ImmersiveSegment(state).steps:getLockedDay9Segment(state)):lockedDay10?getLockedDay10Segment(state):lockedDay11?getLockedDay11Segment(state):lockedDay12?getLockedDay12Segment(state):lockedDay13?getLockedDay13Segment(state):lockedDay14?getLockedDay14Segment(state):lockedDay15?getLockedDay15Segment(state):lockedDay16?getLockedDay16Segment(state):lockedDay17?(day17V4?getDay17V4GameSegment(state):getLockedDay17Segment(state)):lockedDay18?getLockedDay18Segment(state):lockedDay19?getDay19Segment(state):lockedDay20?getLockedDay20Segment(state):lockedDay21?getDay21Segment(state):lockedDay22?getDay22Segment(state):lockedDay23?getDay23Segment(state):lockedDay24?getDay24Segment(state):lockedDay25?getDay25Segment(state):lockedDay26?getDay26Segment(state):lockedDay27?getDay27Segment(state):lockedDay28?getLockedDay28Segment(state):lockedDay29?getDay29Segment(state):lockedDay30?getLockedDay30Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
    const lateStoryDay=lockedDay19&&!day19V4?19:lockedDay20?20:lockedDay21&&!day21V4?21:lockedDay22&&!day22V4?22:lockedDay23&&!day23V4?23:lockedDay24&&!day24V4?24:lockedDay25&&!day25V4?25:lockedDay26&&!day26V4?26:lockedDay27&&!day27V4?27:lockedDay28&&!day28V4?28:lockedDay29&&!day29V4?29:lockedDay30?30:null;
    ...
    if(lockedDay30){const result=applyLockedDay30ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay30Segment(state,result.stage);}
...
function skipImmersiveScene(event) {
  ...
  const resolution=immersiveScene.sequence.find(step=>step.type==="haeunContactResolution"||step.type==="haeunStayResolution"||step.type==="contactConsentCue"||step.type==="lodgingConsentCue"||step.type==="travelConfirmationCue"||step.type==="photoConsentCue"||isDay23V4ResolutionStep(step)||isDay24V4ResolutionStep(step)||isDay25V4ResolutionStep(step)||isDay26V4ResolutionStep(step)||isDay27V4ResolutionStep(step)||isDay28V4ResolutionStep(step)||isDay29V4ResolutionStep(step));
  ...
  if(completionCue){
    try{
      if(completionCue.day===29)completeDay29V4GameChapter(state,completionCue);
...
function finishImmersiveScene() {
  ...
  if(completedSession?.id===LOCKED_DAY30_SCENE_ID&&state.storyFlags?.day30DecisionStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY30_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY30_SCENE_ID,getLockedDay30LegacyChoice(state));state.storyFlags.day30RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY30_SCENE_ID&&state.storyFlags?.day30RuntimeComplete&&!state.storyFlags?.day30FreeActionComplete){const freeAction={type:"freeAction",id:"day30-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
