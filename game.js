...
function openStoryScene(scene) {
  ...
  const freeResume=isStoryFreeActionResume(state,scene.id)&&!(lockedDay23&&day23V4)&&!(lockedDay24&&day24V4)&&!(lockedDay25&&day25V4)&&!(lockedDay26&&day26V4)&&!(lockedDay27&&day27V4)&&!(lockedDay28&&day28V4);
  ...
  const day29Resume=lockedDay29?getLockedDay29ResumePresentation(state):null;
  ...
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:freeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:state.storyFreeAction.phase,location:state.storyFreeAction.location,maxActions:state.storyFreeAction.maxActions},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?(day6V3?getDay6V3ImmersiveSegment(state).steps:getLockedDay6Segment(state)):lockedDay7?(day7V3?getDay7V3ImmersiveSegment(state).steps:getLockedDay7Segment(state)):lockedDay8?(day8V3?getDay8V3ImmersiveSegment(state).steps:getLockedDay8Segment(state)):lockedDay9?(day9V3?getDay9V3ImmersiveSegment(state).steps:getLockedDay9Segment(state)):lockedDay10?getLockedDay10Segment(state):lockedDay11?getLockedDay11Segment(state):lockedDay12?getLockedDay12Segment(state):lockedDay13?getLockedDay13Segment(state):lockedDay14?getLockedDay14Segment(state):lockedDay15?getLockedDay15Segment(state):lockedDay16?getLockedDay16Segment(state):lockedDay17?(day17V4?getDay17V4GameSegment(state):getLockedDay17Segment(state)):lockedDay18?getLockedDay18Segment(state):lockedDay19?getDay19Segment(state):lockedDay20?getLockedDay20Segment(state):lockedDay21?getDay21Segment(state):lockedDay22?getDay22Segment(state):lockedDay23?getDay23Segment(state):lockedDay24?getDay24Segment(state):lockedDay25?getDay25Segment(state):lockedDay26?getDay26Segment(state):lockedDay27?getDay27Segment(state):lockedDay28?getLockedDay28Segment(state):lockedDay29?getLockedDay29Segment(state):lockedDay30?getLockedDay30Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
    const lateStoryDay=lockedDay19&&!day19V4?19:lockedDay20?20:lockedDay21&&!day21V4?21:lockedDay22&&!day22V4?22:lockedDay23&&!day23V4?23:lockedDay24&&!day24V4?24:lockedDay25&&!day25V4?25:lockedDay26&&!day26V4?26:lockedDay27&&!day27V4?27:lockedDay28&&!day28V4?28:lockedDay29?29:lockedDay30?30:null;
    ...
    if(lockedDay29){const result=applyLockedDay29ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay29Segment(state,result.stage);}
...
function startImmersiveScene(session) {
  ...
  if(session.id===LOCKED_DAY29_SCENE_ID){const resumeVisual=getLockedDay29ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
...
function skipImmersiveScene(event) {
  ...
  const resolution=immersiveScene.sequence.find(step=>step.type==="haeunContactResolution"||step.type==="haeunStayResolution"||step.type==="contactConsentCue"||step.type==="lodgingConsentCue"||step.type==="travelConfirmationCue"||step.type==="photoConsentCue"||isDay23V4ResolutionStep(step)||isDay24V4ResolutionStep(step)||isDay25V4ResolutionStep(step)||isDay26V4ResolutionStep(step)||isDay27V4ResolutionStep(step)||isDay28V4ResolutionStep(step));
  ...
  if(completionCue){
    try{
      if(completionCue.day===28)completeDay28V4GameChapter(state,completionCue);
...
function finishImmersiveScene() {
  ...
  if(completedSession?.id===LOCKED_DAY29_SCENE_ID&&state.storyFlags?.day29PlanStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY29_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY29_SCENE_ID,getLockedDay29LegacyChoice(state));state.storyFlags.day29RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY29_SCENE_ID&&state.storyFlags?.day29RuntimeComplete&&!state.storyFlags?.day29FreeActionComplete){const freeAction={type:"freeAction",id:"day29-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
