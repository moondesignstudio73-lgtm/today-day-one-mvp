...
function openStoryScene(scene) {
  ...
  const day1FreeResume=lockedDay1&&state.storyFreeAction?.day===1&&state.storyFreeAction.status!=="COMPLETE";
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:day1FreeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:"evening",location:"hospital",maxActions:1},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?getLockedDay6Segment(state):lockedDay7?getLockedDay7Segment(state):lockedDay8?getLockedDay8Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
