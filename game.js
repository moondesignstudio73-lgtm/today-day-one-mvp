...
import { applyDay25V4GameChoice, applyDay25V4GameResolution, completeDay25V4GameChapter, getDay25V4GameCompatibility, getDay25V4GameResumePresentation, getDay25V4GameSegment, isDay25V4ResolutionStep, prepareDay25V4GameEntry } from "./src/day25-v4-game-bridge.mjs?v=1";
import { getDay25V4RuntimeResolution } from "./src/day25-v4-runtime-resolution.mjs?v=1";
...
function openStoryScene(scene) {
  ...
  const freeResume=isStoryFreeActionResume(state,scene.id)&&!(lockedDay23&&day23V4)&&!(lockedDay24&&day24V4)&&!(lockedDay25&&day25V4);
  ...
  const day26Resume=lockedDay26?getLockedDay26ResumePresentation(state):null;
  ...
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:freeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:state.storyFreeAction.phase,location:state.storyFreeAction.location,maxActions:state.storyFreeAction.maxActions},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?(day6V3?getDay6V3ImmersiveSegment(state).steps:getLockedDay6Segment(state)):lockedDay7?(day7V3?getDay7V3ImmersiveSegment(state).steps:getLockedDay7Segment(state)):lockedDay8?(day8V3?getDay8V3ImmersiveSegment(state).steps:getLockedDay8Segment(state)):lockedDay9?(day9V3?getDay9V3ImmersiveSegment(state).steps:getLockedDay9Segment(state)):lockedDay10?getLockedDay10Segment(state):lockedDay11?getLockedDay11Segment(state):lockedDay12?getLockedDay12Segment(state):lockedDay13?getLockedDay13Segment(state):lockedDay14?getLockedDay14Segment(state):lockedDay15?getLockedDay15Segment(state):lockedDay16?getLockedDay16Segment(state):lockedDay17?(day17V4?getDay17V4GameSegment(state):getLockedDay17Segment(state)):lockedDay18?getLockedDay18Segment(state):lockedDay19?getDay19Segment(state):lockedDay20?getLockedDay20Segment(state):lockedDay21?getDay21Segment(state):lockedDay22?getDay22Segment(state):lockedDay23?getDay23Segment(state):lockedDay24?getDay24Segment(state):lockedDay25?getDay25Segment(state):lockedDay26?getLockedDay26Segment(state):lockedDay27?getLockedDay27Segment(state):lockedDay28?getLockedDay28Segment(state):lockedDay29?getLockedDay29Segment(state):lockedDay30?getLockedDay30Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
    ...
    if(lockedDay26){const result=applyLockedDay26ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay26Segment(state,result.stage);}
...
function skipImmersiveScene(event) {
  ...
  const resolution=immersiveScene.sequence.find(step=>step.type==="haeunContactResolution"||step.type==="haeunStayResolution"||step.type==="contactConsentCue"||step.type==="lodgingConsentCue"||step.type==="travelConfirmationCue"||step.type==="photoConsentCue"||isDay23V4ResolutionStep(step)||isDay24V4ResolutionStep(step)||isDay25V4ResolutionStep(step));
  ...
  if(completionCue){
    try{
      if(completionCue.day===25)completeDay25V4GameChapter(state,completionCue);
...
function finishImmersiveScene() {
  ...
  if(completedSession?.id===LOCKED_DAY26_SCENE_ID&&state.storyFlags?.day26SignatureStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY26_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY26_SCENE_ID,getLockedDay26LegacyChoice(state));state.storyFlags.day26RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY26_SCENE_ID&&state.storyFlags?.day26RuntimeComplete&&!state.storyFlags?.day26FreeActionComplete){const freeAction={type:"freeAction",id:"day26-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
