...
import { applyDay6V3Choice, beginDay6V3, getDay6V3Compatibility } from "./src/day6-v3-runtime.mjs?v=3";
import { getDay6V3ChoiceContinuation, getDay6V3ImmersiveSegment } from "./src/day6-v3-immersive-adapter.mjs?v=1";
import { getDay6V3Presentation } from "./src/day6-v3-presentation-data.mjs?v=1";
...
import { applyDay7V3Choice, beginDay7V3, getDay7V3Compatibility } from "./src/day7-v3-runtime.mjs?v=1";
import { getDay7V3ChoiceContinuation, getDay7V3ImmersiveSegment } from "./src/day7-v3-immersive-adapter.mjs?v=1";
import { getDay7V3Presentation } from "./src/day7-v3-presentation-data.mjs?v=1";
...
import { applyDay8V3Choice, beginDay8V3, getDay8V3Compatibility } from "./src/day8-v3-runtime.mjs?v=1";
import { getDay8V3ChoiceContinuation, getDay8V3ImmersiveSegment } from "./src/day8-v3-immersive-adapter.mjs?v=1";
import { getDay8V3Presentation } from "./src/day8-v3-presentation-data.mjs?v=1";
...
import { applyDay9V3Choice, beginDay9V3, getDay9V3Compatibility } from "./src/day9-v3-runtime.mjs?v=1";
import { getDay9V3ChoiceContinuation, getDay9V3ImmersiveSegment } from "./src/day9-v3-immersive-adapter.mjs?v=1";
import { getDay9V3Presentation } from "./src/day9-v3-presentation-data.mjs?v=1";
import { settleDay9V3Transactions } from "./src/day9-v3-transaction-adapter.mjs?v=1";
import { LOCKED_DAY10_SCENE_ID, applyLockedDay10ChoiceState, getLockedDay10LegacyChoice, getLockedDay10ResumePresentation as getLegacyDay10ResumePresentation, getLockedDay10Segment as getLegacyDay10Segment } from "./src/day10-campaign-runtime.mjs?v=2";
import { applyDay10V3Choice, applyDay10V3FollowUpChoice, beginDay10V3, getDay10V3Compatibility } from "./src/day10-v3-runtime.mjs?v=1";
import { getDay10V3ChoiceContinuation, getDay10V3ImmersiveSegment } from "./src/day10-v3-immersive-adapter.mjs?v=1";
import { getDay10V3Presentation } from "./src/day10-v3-presentation-data.mjs?v=1";
import { LOCKED_DAY11_SCENE_ID, applyLockedDay11ChoiceState, getLockedDay11LegacyChoice, getLockedDay11ResumePresentation as getLegacyDay11ResumePresentation, getLockedDay11Segment as getLegacyDay11Segment } from "./src/day11-campaign-runtime.mjs?v=2";
import { applyDay11V3Choice, beginDay11V3, getDay11V3Compatibility } from "./src/day11-v3-runtime.mjs?v=1";
import { getDay11V3ChoiceContinuation, getDay11V3ImmersiveSegment, getDay11V3ResumePresentation } from "./src/day11-v3-immersive-adapter.mjs?v=1";
import { LOCKED_DAY12_SCENE_ID, applyLockedDay12ChoiceState, getLockedDay12LegacyChoice, getLockedDay12ResumePresentation as getLegacyDay12ResumePresentation, getLockedDay12Segment as getLegacyDay12Segment } from "./src/day12-campaign-runtime.mjs?v=3";
import { applyDay12V3Choice, beginDay12V3, getDay12V3Compatibility } from "./src/day12-v3-runtime.mjs?v=1";
import { getDay12V3ChoiceContinuation, getDay12V3ImmersiveSegment, getDay12V3ResumePresentation } from "./src/day12-v3-immersive-adapter.mjs?v=2";
import { LOCKED_DAY13_SCENE_ID, applyLockedDay13ChoiceState, getLockedDay13LegacyChoice, getLockedDay13ResumePresentation as getLegacyDay13ResumePresentation, getLockedDay13Segment as getLegacyDay13Segment } from "./src/day13-campaign-runtime.mjs?v=2";
import { applyDay13V3Choice, beginDay13V3, getDay13V3Compatibility } from "./src/day13-v3-runtime.mjs?v=1";
import { getDay13V3ChoiceContinuation, getDay13V3ImmersiveSegment, getDay13V3ResumePresentation } from "./src/day13-v3-immersive-adapter.mjs?v=1";
import { LOCKED_DAY14_SCENE_ID, applyLockedDay14ChoiceState as applyLegacyDay14ChoiceState, getLockedDay14LegacyChoice, getLockedDay14ResumePresentation as getLegacyDay14ResumePresentation, getLockedDay14Segment as getLegacyDay14Segment } from "./src/day14-campaign-runtime.mjs?v=2";
import { beginDay14V4, getDay14V4Compatibility } from "./src/day14-v4-state-contract.mjs?v=1";
import { applyDay14V4Choice } from "./src/day14-v4-runtime.mjs?v=1";
import { getDay14V4ChoiceContinuation, getDay14V4ImmersiveSegment, getDay14V4ResumePresentation } from "./src/day14-v4-immersive-adapter.mjs?v=1";
import { LOCKED_DAY15_SCENE_ID, applyLockedDay15ChoiceState, getLockedDay15LegacyChoice, getLockedDay15ResumePresentation as getLegacyDay15ResumePresentation, getLockedDay15Segment as getLegacyDay15Segment } from "./src/day15-campaign-runtime.mjs?v=2";
import { applyDay15V4GameChoice, completeDay15V4GameChapter, getDay15V4Compatibility, getDay15V4GameResumePresentation, getDay15V4GameSegment, prepareDay15V4GameEntry } from "./src/day15-v4-game-bridge.mjs?v=1";
import { LOCKED_DAY16_SCENE_ID, applyLockedDay16ChoiceState, getLockedDay16LegacyChoice, getLockedDay16ResumePresentation as getLegacyDay16ResumePresentation, getLockedDay16Segment as getLegacyDay16Segment } from "./src/day16-campaign-runtime.mjs?v=2";
import { applyDay16V4GameChoice, completeDay16V4GameChapter, getDay16V4Compatibility, getDay16V4GameResumePresentation, getDay16V4GameSegment, prepareDay16V4GameEntry } from "./src/day16-v4-game-bridge.mjs?v=1";
...
import { applyDay22V4GameChoice, applyDay22V4GameResolution, completeDay22V4GameChapter, getDay22V4GameCompatibility, getDay22V4GameResumePresentation, getDay22V4GameSegment, prepareDay22V4GameEntry } from "./src/day22-v4-game-bridge.mjs?v=7";
import { getDay22V4RuntimeResolution } from "./src/day22-v4-runtime-resolution.mjs?v=2";
...
import { applyDay23V4GameChoice, applyDay23V4GameResolution, completeDay23V4GameChapter, getDay23V4GameCompatibility, getDay23V4GameResumePresentation, getDay23V4GameSegment, isDay23V4ResolutionStep, prepareDay23V4GameEntry } from "./src/day23-v4-game-bridge.mjs?v=2";
import { getDay23V4RuntimeResolution } from "./src/day23-v4-runtime-resolution.mjs?v=1";
...
import { applyDay24V4GameChoice, applyDay24V4GameResolution, completeDay24V4GameChapter, getDay24V4GameCompatibility, getDay24V4GameResumePresentation, getDay24V4GameSegment, isDay24V4ResolutionStep, prepareDay24V4GameEntry } from "./src/day24-v4-game-bridge.mjs?v=1";
import { getDay24V4RuntimeResolution } from "./src/day24-v4-runtime-resolution.mjs?v=1";
...
import { applyDay25V4GameChoice, applyDay25V4GameResolution, completeDay25V4GameChapter, getDay25V4GameCompatibility, getDay25V4GameResumePresentation, getDay25V4GameSegment, isDay25V4ResolutionStep, prepareDay25V4GameEntry } from "./src/day25-v4-game-bridge.mjs?v=3";
import { getDay25V4RuntimeResolution } from "./src/day25-v4-runtime-resolution.mjs?v=2";
import { LOCKED_DAY26_SCENE_ID, applyLockedDay26ChoiceState, getLockedDay26LegacyChoice, getLockedDay26ResumePresentation, getLockedDay26Segment } from "./src/day26-campaign-runtime.mjs?v=1";
import { applyDay26V4GameChoice, applyDay26V4GameResolution, completeDay26V4GameChapter, getDay26V4GameCompatibility, getDay26V4GameResumePresentation, getDay26V4GameSegment, isDay26V4ResolutionStep, prepareDay26V4GameEntry } from "./src/day26-v4-game-bridge.mjs?v=3";
import { getDay26V4RuntimeResolution } from "./src/day26-v4-runtime-resolution.mjs?v=1";
import { LOCKED_DAY27_SCENE_ID, applyLockedDay27ChoiceState, getLockedDay27LegacyChoice, getLockedDay27ResumePresentation, getLockedDay27Segment } from "./src/day27-campaign-runtime.mjs?v=1";
...
import { applyDay28V4GameChoice, applyDay28V4GameResolution, completeDay28V4GameChapter, getDay28V4GameCompatibility, getDay28V4GameResumePresentation, getDay28V4GameSegment, isDay28V4ResolutionStep, prepareDay28V4GameEntry } from "./src/day28-v4-game-bridge.mjs?v=11";
import { getDay28V4RuntimeResolution } from "./src/day28-v4-runtime-resolution.mjs?v=3";
...
import { applyDay29V4GameChoice, applyDay29V4GameResolution, completeDay29V4GameChapter, getDay29V4GameCompatibility, getDay29V4GameResumePresentation, getDay29V4GameSegment, isDay29V4ResolutionStep, prepareDay29V4GameEntry } from "./src/day29-v4-game-bridge.mjs?v=3";
import { getDay29V4RuntimeResolution } from "./src/day29-v4-runtime-resolution.mjs?v=2";
import { LOCKED_DAY30_SCENE_ID, applyLockedDay30ChoiceState as applyLegacyDay30ChoiceState, getLockedDay30LegacyChoice, getLockedDay30ResumePresentation, getLockedDay30Segment as getLegacyDay30Segment } from "./src/day30-campaign-runtime.mjs?v=1";
import { applyDay30V4GameChoice, applyDay30V4GameResolution, completeDay30V4GameChapter, getDay30V4GameCompatibility, getDay30V4GameResumePresentation, getDay30V4GameSegment, isDay30V4ResolutionStep, prepareDay30V4GameEntry } from "./src/day30-v4-game-bridge.mjs?v=1";
import { getDay30V4RuntimeResolution } from "./src/day30-v4-runtime-resolution.mjs?v=1";
...
import { getBackgroundAsset, getGiftVehicleAsset, getNpcSprite } from "./src/assets/asset-manifest.mjs?v=25";
import { isInternalStoryStep, isPlayerFacingStoryStep } from "./src/story-player-facing-policy.mjs?v=1";
import { recordLateStoryV4Choice } from "./src/late-story-v4-expansion.mjs?v=1";
import { getAvailableStoryChoices, getStoryScene, resolveStoryChoice, selectNextStoryScene } from "./src/story-manager.mjs?v=33";
...
import { getActionResultAsset, getHighTrustActionResultAsset, getVisibleActionEffects } from "./src/action-result-assets.mjs?v=15";
...
const getLockedDay14Segment=state=>["V4","V4_NEW"].includes(getDay14V4Compatibility(state).mode)?getDay14V4ImmersiveSegment(state).steps:getLegacyDay14Segment(state);
...
function renderStoryCampaignStandby(){
  document.body.classList.remove("ui-classic-mode","ui-night-mode");document.body.classList.add("ui-story-mode");$("#gameScreen").classList.remove("classic-mode","night-mode");$("#gameScreen").classList.add("story-mode");$("#nightHome").classList.add("hidden");$(".play-panel").classList.remove("hidden");
  if(!immersiveScene&&!$("#storyFreeActionLayer")?.classList.contains("hidden")){enforceModeExclusiveUi("campaign-free-action");return;}
  if(!immersiveScene){$("#sceneTitle").textContent="STORY MODE";$("#sceneTitle").classList.remove("hidden");$("#sceneText").textContent="다음 장면을 준비하고 있습니다.";$("#visualNovelStage").classList.add("narration-mode");}
  enforceModeExclusiveUi("campaign-standby");
}
...
const DEFAULT_INTRO_VIDEO_PLAYLIST = ["assets/video/free-mode-prologue.mp4"];
...
function getDay6V3RelationshipBand(currentState){
  const affection=Number(currentState?.affection)||0,trust=Number(currentState?.trust)||0;
  if(affection>=800&&trust>=700)return "VERY_HIGH";
  if(affection>=650&&trust>=550)return "HIGH";
  if(affection>=350&&trust>=300)return "MID";
  return "LOW";
}

function getLockedDay10ResumePresentation(currentState){
  if(getDay10V3Compatibility(currentState).mode==="V1_LEGACY")return getLegacyDay10ResumePresentation(currentState);
  const sceneNumber=currentState.storyFlags?.day10V3SceneCheckpoint??1,presentation=getDay10V3Presentation(sceneNumber,currentState);
  const backgroundId=sceneNumber===5?"neighborhood-market-day":sceneNumber>=13?"home-night":"home-morning";
  return {...presentation,backgroundId};
}

function getLockedDay10Segment(currentState){return getDay10V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay10Segment(currentState):getDay10V3ImmersiveSegment(currentState).steps;}

function getLockedDay11ResumePresentation(currentState){return getDay11V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay11ResumePresentation(currentState):getDay11V3ResumePresentation(currentState);}

function getLockedDay11Segment(currentState){return getDay11V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay11Segment(currentState):getDay11V3ImmersiveSegment(currentState).steps;}

function getLockedDay12ResumePresentation(currentState){return getDay12V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay12ResumePresentation(currentState):getDay12V3ResumePresentation(currentState);}

function getLockedDay12Segment(currentState){return getDay12V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay12Segment(currentState):getDay12V3ImmersiveSegment(currentState).steps;}

function getLockedDay13ResumePresentation(currentState){return getDay13V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay13ResumePresentation(currentState):getDay13V3ResumePresentation(currentState);}

function getLockedDay13Segment(currentState){return getDay13V3Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay13Segment(currentState):getDay13V3ImmersiveSegment(currentState).steps;}

function getLockedDay14ResumePresentation(currentState){return getDay14V4Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay14ResumePresentation(currentState):getDay14V4ResumePresentation(currentState);}

function getLockedDay15ResumePresentation(currentState){return getDay15V4Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay15ResumePresentation(currentState):getDay15V4GameResumePresentation(currentState);}

function getLockedDay15Segment(currentState){return getDay15V4Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay15Segment(currentState):getDay15V4GameSegment(currentState);}

function getLockedDay16ResumePresentation(currentState){return getDay16V4Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay16ResumePresentation(currentState):getDay16V4GameResumePresentation(currentState);}

function getLockedDay16Segment(currentState){return getDay16V4Compatibility(currentState).mode==="V1_LEGACY"?getLegacyDay16Segment(currentState):getDay16V4GameSegment(currentState);}

function getLockedDay18ResumePresentation(currentState){return currentState.storyFlags?.day18V4?getDay18V4GameResumePresentation(currentState):getLegacyDay18ResumePresentation(currentState);}
function getLockedDay18Segment(currentState,stage){return currentState.storyFlags?.day18V4?getDay18V4GameSegment(currentState):getLegacyDay18Segment(currentState,stage);}
...
function getDay23ResumePresentation(currentState){return currentState.storyFlags?.day23V4?getDay23V4GameResumePresentation(currentState):getLockedDay23ResumePresentation(currentState);}
function getDay23Segment(currentState){return currentState.storyFlags?.day23V4?getDay23V4GameSegment(currentState):getLockedDay23Segment(currentState);}
function applyDay23ChoiceState(currentState,id){if(!currentState.storyFlags?.day23V4)return applyLockedDay23ChoiceState(currentState,id);applyDay23V4GameChoice(currentState,id);return {stage:0};}
function getDay24ResumePresentation(currentState){return currentState.storyFlags?.day24V4?getDay24V4GameResumePresentation(currentState):getLockedDay24ResumePresentation(currentState);}
function getDay24Segment(currentState){return currentState.storyFlags?.day24V4?getDay24V4GameSegment(currentState):getLockedDay24Segment(currentState);}
function applyDay24ChoiceState(currentState,id){if(!currentState.storyFlags?.day24V4)return applyLockedDay24ChoiceState(currentState,id);applyDay24V4GameChoice(currentState,id);return {stage:0};}
function getDay25ResumePresentation(currentState){return currentState.storyFlags?.day25V4?getDay25V4GameResumePresentation(currentState):getLockedDay25ResumePresentation(currentState);}
function getDay25Segment(currentState){return currentState.storyFlags?.day25V4?getDay25V4GameSegment(currentState):getLockedDay25Segment(currentState);}
function applyDay25ChoiceState(currentState,id){if(!currentState.storyFlags?.day25V4)return applyLockedDay25ChoiceState(currentState,id);applyDay25V4GameChoice(currentState,id);return {stage:0};}
function getDay26ResumePresentation(currentState){return currentState.storyFlags?.day26V4?getDay26V4GameResumePresentation(currentState):getLockedDay26ResumePresentation(currentState);}
function getDay26Segment(currentState){return currentState.storyFlags?.day26V4?getDay26V4GameSegment(currentState):getLockedDay26Segment(currentState);}
function applyDay26ChoiceState(currentState,id){if(!currentState.storyFlags?.day26V4)return applyLockedDay26ChoiceState(currentState,id);applyDay26V4GameChoice(currentState,id);return {stage:0};}
function getDay27ResumePresentation(currentState){return currentState.storyFlags?.day27V4?getDay27V4GameResumePresentation(currentState):getLockedDay27ResumePresentation(currentState);}
function getDay27Segment(currentState){return currentState.storyFlags?.day27V4?getDay27V4GameSegment(currentState):getLockedDay27Segment(currentState);}
function getDay28ResumePresentation(currentState){return currentState.storyFlags?.day28V4?getDay28V4GameResumePresentation(currentState):getLockedDay28ResumePresentation(currentState);}
function getDay28Segment(currentState){return currentState.storyFlags?.day28V4?getDay28V4GameSegment(currentState):getLockedDay28Segment(currentState);}
function getLockedDay28Segment(currentState,stage){return currentState.storyFlags?.day28V4?getDay28V4GameSegment(currentState):getLegacyDay28Segment(currentState,stage);}
function getDay29ResumePresentation(currentState){return currentState.storyFlags?.day29V4?getDay29V4GameResumePresentation(currentState):getLockedDay29ResumePresentation(currentState);}
function getDay29Segment(currentState){return currentState.storyFlags?.day29V4?getDay29V4GameSegment(currentState):getLockedDay29Segment(currentState);}
function getDay30ResumePresentation(currentState){return currentState.storyFlags?.day30V4?getDay30V4GameResumePresentation(currentState):getLockedDay30ResumePresentation(currentState);}
function getDay30Segment(currentState){return currentState.storyFlags?.day30V4?getDay30V4GameSegment(currentState):getLockedDay30Segment(currentState);}
function getLockedDay30Segment(currentState,stage){return currentState.storyFlags?.day30V4?getDay30V4GameSegment(currentState):getLegacyDay30Segment(currentState,stage);}
function applyLockedDay30ChoiceState(currentState,id){if(!currentState.storyFlags?.day30V4)return applyLegacyDay30ChoiceState(currentState,id);applyDay30V4GameChoice(currentState,id);return {stage:0};}
...
function openStoryScene(scene) {
  ...
  const lockedDay8=scene.id===LOCKED_DAY8_SCENE_ID;
  const lockedDay9=scene.id===LOCKED_DAY9_SCENE_ID;
  const lockedDay10=scene.id===LOCKED_DAY10_SCENE_ID;
  const lockedDay11=scene.id===LOCKED_DAY11_SCENE_ID;
  const lockedDay12=scene.id===LOCKED_DAY12_SCENE_ID;
  const lockedDay13=scene.id===LOCKED_DAY13_SCENE_ID;
  const lockedDay14=scene.id===LOCKED_DAY14_SCENE_ID;
  const lockedDay15=scene.id===LOCKED_DAY15_SCENE_ID;
  const lockedDay16=scene.id===LOCKED_DAY16_SCENE_ID;
  const lockedDay17=scene.id===LOCKED_DAY17_SCENE_ID;
  const lockedDay18=scene.id===LOCKED_DAY18_SCENE_ID;
  const lockedDay19=scene.id===LOCKED_DAY19_SCENE_ID;
  const lockedDay20=scene.id===LOCKED_DAY20_SCENE_ID;
  const lockedDay21=scene.id===LOCKED_DAY21_SCENE_ID;
  const lockedDay22=scene.id===LOCKED_DAY22_SCENE_ID;
  const lockedDay23=scene.id===LOCKED_DAY23_SCENE_ID;
  const lockedDay24=scene.id===LOCKED_DAY24_SCENE_ID;
  const lockedDay25=scene.id===LOCKED_DAY25_SCENE_ID;
  const lockedDay26=scene.id===LOCKED_DAY26_SCENE_ID;
  const lockedDay27=scene.id===LOCKED_DAY27_SCENE_ID;
  const lockedDay28=scene.id===LOCKED_DAY28_SCENE_ID;
  const lockedDay29=scene.id===LOCKED_DAY29_SCENE_ID;
  const lockedDay30=scene.id===LOCKED_DAY30_SCENE_ID;
  const day6V3=lockedDay6&&getDay6V3Compatibility(state).mode!=="V1_LEGACY";
  const day7V3=lockedDay7&&getDay7V3Compatibility(state).mode!=="V1_LEGACY";
  const day8V3=lockedDay8&&getDay8V3Compatibility(state).mode!=="V1_LEGACY";
  const day9V3=lockedDay9&&getDay9V3Compatibility(state).mode!=="V1_LEGACY";
  const day10V3=lockedDay10&&getDay10V3Compatibility(state).mode!=="V1_LEGACY";
  const day11V3=lockedDay11&&getDay11V3Compatibility(state).mode!=="V1_LEGACY";
  const day12V3=lockedDay12&&getDay12V3Compatibility(state).mode!=="V1_LEGACY";
  const day13V3=lockedDay13&&getDay13V3Compatibility(state).mode!=="V1_LEGACY";
  const day14Compatibility=lockedDay14?getDay14V4Compatibility(state):null;
  const day14Blocked=day14Compatibility?.mode==="BLOCKED_PREREQUISITE";
  const day14V4=lockedDay14&&!['V1_LEGACY','BLOCKED_PREREQUISITE'].includes(day14Compatibility?.mode);
  const day15Compatibility=lockedDay15?getDay15V4Compatibility(state):null;
  const day15Blocked=["BLOCKED_PREREQUISITE","BLOCKED_CORRUPT"].includes(day15Compatibility?.mode);
  const day15V4=lockedDay15&&["V4_NEW","V4"].includes(day15Compatibility?.mode);
  const day16Compatibility=lockedDay16?getDay16V4Compatibility(state):null;
  const day16Blocked=["BLOCKED_PREREQUISITE","BLOCKED_CORRUPT"].includes(day16Compatibility?.mode);
  const day16V4=lockedDay16&&["V4_NEW","V4"].includes(day16Compatibility?.mode);
  const day17Compatibility=lockedDay17?getDay17V4Compatibility(state):null;
  const day17Blocked=["BLOCKED_PREREQUISITE","BLOCKED_CORRUPT"].includes(day17Compatibility?.mode);
  const day17V4=lockedDay17&&["V4_NEW","V4"].includes(day17Compatibility?.mode);
  ...
  const day23Compatibility=lockedDay23?getDay23V4GameCompatibility(state):null;
  const day23Blocked=lockedDay23&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day23Compatibility?.mode);
  const day23V4=lockedDay23&&["V4_NEW","V4"].includes(day23Compatibility?.mode);
  const day24Compatibility=lockedDay24?getDay24V4GameCompatibility(state):null;
  const day24Blocked=lockedDay24&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day24Compatibility?.mode);
  const day24V4=lockedDay24&&["V4_NEW","V4"].includes(day24Compatibility?.mode);
  const day25Compatibility=lockedDay25?getDay25V4GameCompatibility(state):null;
  const day25Blocked=lockedDay25&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day25Compatibility?.mode);
  const day25V4=lockedDay25&&["V4_NEW","V4"].includes(day25Compatibility?.mode);
  const day26Compatibility=lockedDay26?getDay26V4GameCompatibility(state):null;
  const day26Blocked=lockedDay26&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day26Compatibility?.mode);
  const day26V4=lockedDay26&&["V4_NEW","V4"].includes(day26Compatibility?.mode);
  const day27Compatibility=lockedDay27?getDay27V4GameCompatibility(state):null;
  const day27Blocked=lockedDay27&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day27Compatibility?.mode);
  const day27V4=lockedDay27&&["V4_NEW","V4"].includes(day27Compatibility?.mode);
  const day28Compatibility=lockedDay28?getDay28V4GameCompatibility(state):null;
  const day28Blocked=lockedDay28&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day28Compatibility?.mode);
  const day28V4=lockedDay28&&["V4_NEW","V4"].includes(day28Compatibility?.mode);
  const day29Compatibility=lockedDay29?getDay29V4GameCompatibility(state):null;
  const day29Blocked=lockedDay29&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day29Compatibility?.mode);
  const day29V4=lockedDay29&&["V4_NEW","V4"].includes(day29Compatibility?.mode);
  const day30Compatibility=lockedDay30?getDay30V4GameCompatibility(state):null;
  const day30Blocked=lockedDay30&&["BLOCKED_PREREQUISITE","INVALID_V4"].includes(day30Compatibility?.mode);
  const day30V4=lockedDay30&&["V4_NEW","V4"].includes(day30Compatibility?.mode);
  ...
  if(day23Blocked){toast("DAY 22 V4 완료 기록과 DAY 23 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day24Blocked){toast("DAY 23 V4 완료 기록과 DAY 24 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day25Blocked){toast("DAY 24 V4 완료 기록과 DAY 25 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day26Blocked){toast("DAY 25 V4 완료 기록과 DAY 26 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day27Blocked){toast("DAY 26 V4 완료 기록과 DAY 27 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day28Blocked){toast("DAY 27 V4 완료 기록과 DAY 28 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day29Blocked){toast("DAY 28 V4 완료 기록과 DAY 29 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  if(day30Blocked){toast("DAY 29 V4 완료 기록과 DAY 30 연결 상태를 확인해야 합니다. 기존 저장은 변경하지 않았습니다.");return;}
  ...
  if(day9V3&&getDay9V3Compatibility(state).mode==="V3_NEW")beginDay9V3(state,{relationshipBand:getDay6V3RelationshipBand(state),priorLightContact:state.storyFlags?.day7V3HandContactEstablished===true,unresolvedKnownConflict:state.storyFlags?.day7V3TrustLow===true||state.storyFlags?.day7V3LieRecorded===true});
  if(day10V3&&getDay10V3Compatibility(state).mode==="V3_NEW")beginDay10V3(state,{relationshipBand:getDay6V3RelationshipBand(state),haeunAvailableForDinner:state.storyFlags?.day9V3DistanceRemaining!==true&&state.storyFlags?.day9V3UnresolvedKnownConflict!==true,greenShirtOwned:["GIFT_ACCEPTED","HAEUN_SELF_PURCHASED"].includes(state.storyFlags?.day9V3GreenShirtState),greenShirtWornToday:state.girlfriendEquipment?.clothes==="day9-green-pocket-shirt",day9TopTriedOn:state.storyFlags?.day9V3PlayerClothingState!=="NOT_TRIED",priorHandHold:state.storyFlags?.day7V3HandContactEstablished===true});
  if(day11V3&&getDay11V3Compatibility(state).mode==="V3_NEW")beginDay11V3(state,{relationshipBand:getDay6V3RelationshipBand(state)});
  if(day12V3&&getDay12V3Compatibility(state).mode==="V3_NEW")beginDay12V3(state,{relationshipBand:getDay6V3RelationshipBand(state)});
  if(day13V3&&getDay13V3Compatibility(state).mode==="V3_NEW")beginDay13V3(state,{relationshipBand:getDay6V3RelationshipBand(state)});
  if(day14V4&&getDay14V4Compatibility(state).mode==="V4_NEW")beginDay14V4(state,{relationshipBand:getDay6V3RelationshipBand(state),priorHandContact:state.storyFlags?.day7V3HandContactEstablished===true,unresolvedContactBoundary:state.storyFlags?.day13V3HaeunDisclosureMismatch===true||state.storyFlags?.day13V3HaeunNeedsSpace===true,day13DeskPhotoReceived:state.storyFlags?.day13V3Day22Callback??null,day13FlowerMessageReceived:state.storyFlags?.day13V3FlowerInterest===true,canAffordFlower:state.money>=25000,haeunMeetingAvailability:state.storyFlags?.day13V3HaeunNeedsSpace===true?"UNAVAILABLE":"AVAILABLE",haeunCallsLater:state.storyFlags?.day13V3HaeunNeedsSpace!==true,exhibitionInviteEligible:state.storyFlags?.day13V3HaeunDisclosureMismatch!==true&&state.storyFlags?.day13V3HaeunNeedsSpace!==true});
  if(day15V4)prepareDay15V4GameEntry(state,{relationshipBand:getDay6V3RelationshipBand(state),haeunCallsTonight:state.storyFlags?.day14V4HaeunCallsLater===true});
  if(day16V4)prepareDay16V4GameEntry(state);
  ...
  if(day23V4)prepareDay23V4GameEntry(state);
  if(day24V4)prepareDay24V4GameEntry(state);
  if(day25V4)prepareDay25V4GameEntry(state);
  if(day26V4)prepareDay26V4GameEntry(state);
  if(day27V4)prepareDay27V4GameEntry(state);
  if(day28V4)prepareDay28V4GameEntry(state);
  if(day29V4)prepareDay29V4GameEntry(state);
  if(day30V4)prepareDay30V4GameEntry(state);
  const presentation=resolveStoryPresentation(scene,state);
  const freeResume=isStoryFreeActionResume(state,scene.id)&&!(lockedDay23&&day23V4)&&!(lockedDay24&&day24V4)&&!(lockedDay25&&day25V4)&&!(lockedDay26&&day26V4)&&!(lockedDay27&&day27V4)&&!(lockedDay28&&day28V4)&&!(lockedDay29&&day29V4)&&!(lockedDay30&&day30V4);
  ...
  const day6Resume=lockedDay6?(day6V3?getDay6V3Presentation(state.storyFlags?.day6V3SceneCheckpoint??1):getLockedDay6ResumePresentation(state)):null;
  const day7Resume=lockedDay7?(day7V3?getDay7V3Presentation(state.storyFlags?.day7V3SceneCheckpoint??1,state):getLockedDay7ResumePresentation(state)):null;
  const day8Resume=lockedDay8?(day8V3?getDay8V3Presentation(state.storyFlags?.day8V3SceneCheckpoint??1,state):getLockedDay8ResumePresentation(state)):null;
  const day9Resume=lockedDay9?(day9V3?getDay9V3Presentation(state.storyFlags?.day9V3SceneCheckpoint??1,state):getLockedDay9ResumePresentation(state)):null;
  const day10Resume=lockedDay10?getLockedDay10ResumePresentation(state):null;
  const day11Resume=lockedDay11?getLockedDay11ResumePresentation(state):null;
  const day12Resume=lockedDay12?getLockedDay12ResumePresentation(state):null;
  const day13Resume=lockedDay13?getLockedDay13ResumePresentation(state):null;
  const day14Resume=lockedDay14?(day14V4?getDay14V4ResumePresentation(state):getLegacyDay14ResumePresentation(state)):null;
  const day15Resume=lockedDay15?getLockedDay15ResumePresentation(state):null;
  const day16Resume=lockedDay16?getLockedDay16ResumePresentation(state):null;
  const day17Resume=lockedDay17?(day17V4?getDay17V4GameResumePresentation(state):getLockedDay17ResumePresentation(state)):null;
  const day18Resume=lockedDay18?getLockedDay18ResumePresentation(state):null;
  ...
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:freeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:state.storyFreeAction.phase,location:state.storyFreeAction.location,maxActions:state.storyFreeAction.maxActions},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?(day6V3?getDay6V3ImmersiveSegment(state).steps:getLockedDay6Segment(state)):lockedDay7?(day7V3?getDay7V3ImmersiveSegment(state).steps:getLockedDay7Segment(state)):lockedDay8?(day8V3?getDay8V3ImmersiveSegment(state).steps:getLockedDay8Segment(state)):lockedDay9?(day9V3?getDay9V3ImmersiveSegment(state).steps:getLockedDay9Segment(state)):lockedDay10?getLockedDay10Segment(state):lockedDay11?getLockedDay11Segment(state):lockedDay12?getLockedDay12Segment(state):lockedDay13?getLockedDay13Segment(state):lockedDay14?getLockedDay14Segment(state):lockedDay15?getLockedDay15Segment(state):lockedDay16?getLockedDay16Segment(state):lockedDay17?(day17V4?getDay17V4GameSegment(state):getLockedDay17Segment(state)):lockedDay18?getLockedDay18Segment(state):lockedDay19?getDay19Segment(state):lockedDay20?getLockedDay20Segment(state):lockedDay21?getDay21Segment(state):lockedDay22?getDay22Segment(state):lockedDay23?getDay23Segment(state):lockedDay24?getDay24Segment(state):lockedDay25?getDay25Segment(state):lockedDay26?getDay26Segment(state):lockedDay27?getDay27Segment(state):lockedDay28?getLockedDay28Segment(state):lockedDay29?getDay29Segment(state):lockedDay30?getLockedDay30Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
    ...
    if(lockedDay6){if(day6V3){const result=applyDay6V3Choice(state,choiceId);SaveManager.save(state);return getDay6V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay6ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay6Segment(state,result.stage);}
    if(lockedDay7){if(day7V3){const result=applyDay7V3Choice(state,choiceId);SaveManager.save(state);return getDay7V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay7ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay7Segment(state,result.stage);}
    if(lockedDay8){if(day8V3){const result=applyDay8V3Choice(state,choiceId);SaveManager.save(state);return getDay8V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay8ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay8Segment(state,result.stage);}
    if(lockedDay9){if(day9V3){const result=applyDay9V3Choice(state,choiceId);if(result.choiceNumber===9)settleDay9V3Transactions(state,{greenGiftAccepted:choiceId==="checkout9_offer_green_gift"&&state.storyFlags?.day9V3DistanceRemaining!==true,haeunSelfPurchase:choiceId==="checkout9_each_buys_own",playerPurchase:choiceId==="checkout9_each_buys_own"});SaveManager.save(state);return getDay9V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay9ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay9Segment(state,result.stage);}
    if(lockedDay10){if(day10V3){const followUp=choiceId.startsWith("followup10_");const result=followUp?applyDay10V3FollowUpChoice(state,choiceId):applyDay10V3Choice(state,choiceId);SaveManager.save(state);return getDay10V3ChoiceContinuation(state,followUp?"FOLLOW_UP":result.choiceNumber);}const result=applyLockedDay10ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay10Segment(state,result.stage);}
    if(lockedDay11){if(day11V3){const result=applyDay11V3Choice(state,choiceId);SaveManager.save(state);return getDay11V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay11ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay11Segment(state,result.stage);}
    if(lockedDay12){if(day12V3){const result=applyDay12V3Choice(state,choiceId);SaveManager.save(state);return getDay12V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay12ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay12Segment(state,result.stage);}
    if(lockedDay13){if(day13V3){const result=applyDay13V3Choice(state,choiceId);SaveManager.save(state);return getDay13V3ChoiceContinuation(state,result.choiceNumber);}const result=applyLockedDay13ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay13Segment(state,result.stage);}
    if(lockedDay14){if(day14V4){const result=applyDay14V4Choice(state,choiceId);SaveManager.save(state);return getDay14V4ChoiceContinuation(state,result.choiceNumber);}const result=applyLegacyDay14ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLegacyDay14Segment(state,result.stage);}
    if(lockedDay15){if(day15V4){const {result,steps}=applyDay15V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay15ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLegacyDay15Segment(state,result.stage);}
    if(lockedDay16){if(day16V4){const {result,steps}=applyDay16V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay16ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLegacyDay16Segment(state,result.stage);}
    if(lockedDay17){if(day17V4){const {result,steps}=applyDay17V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay17ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay17Segment(state,result.stage);}
    if(lockedDay18){const result=applyLockedDay18ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay18Segment(state,result.stage);}
    ...
    if(lockedDay23){const result=applyDay23ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getDay23Segment(state);}
    if(lockedDay24){const result=applyDay24ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getDay24Segment(state);}
    if(lockedDay25){const result=applyDay25ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getDay25Segment(state);}
    if(lockedDay26){const result=applyDay26ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getDay26Segment(state);}
    if(lockedDay27){if(day27V4){const {steps}=applyDay27V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay27ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay27Segment(state,result.stage);}
    if(lockedDay28){if(day28V4){const {steps}=applyDay28V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay28ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay28Segment(state,result.stage);}
    if(lockedDay29){if(day29V4){const {steps}=applyDay29V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay29ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay29Segment(state,result.stage);}
    if(lockedDay30){if(day30V4){const {steps}=applyDay30V4GameChoice(state,choiceId);SaveManager.save(state);return steps;}const result=applyLockedDay30ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay30Segment(state,result.stage);}
...
function startImmersiveScene(session) {
  ...
  if(initialPresentation.backgroundId!==session.presentation?.backgroundId){const firstVisual=session.sequence.find(step=>step.backgroundId===initialPresentation.backgroundId);initialPresentation.backgroundUrl=firstVisual?.backgroundUrl??getBackgroundAsset(initialPresentation.backgroundId);}
  ...
  if(session.id===LOCKED_DAY6_SCENE_ID&&state.storyFlags?.day6V3Version!==3){
  ...
  if(session.id===LOCKED_DAY8_SCENE_ID&&getDay8V3Compatibility(state).mode==="V1_LEGACY"){
    const resumeVisual=getLockedDay8ResumePresentation(state);
    immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};
    immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;
  }
  if(session.id===LOCKED_DAY9_SCENE_ID&&getDay9V3Compatibility(state).mode==="V1_LEGACY"){
    const resumeVisual=getLockedDay9ResumePresentation(state);
    immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};
    immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;
  }
  if(session.id===LOCKED_DAY10_SCENE_ID&&getDay10V3Compatibility(state).mode==="V1_LEGACY"){
    const resumeVisual=getLockedDay10ResumePresentation(state);
    immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};
    immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;
  }
  if(session.id===LOCKED_DAY11_SCENE_ID&&getDay11V3Compatibility(state).mode==="V1_LEGACY"){
    const resumeVisual=getLockedDay11ResumePresentation(state);
    immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};
    immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;
  }
  if(session.id===LOCKED_DAY12_SCENE_ID&&getDay12V3Compatibility(state).mode==="V1_LEGACY"){
    const resumeVisual=getLockedDay12ResumePresentation(state);
    immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};
    immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;
  }
  if(session.id===LOCKED_DAY13_SCENE_ID&&getDay13V3Compatibility(state).mode==="V1_LEGACY"){
    const resumeVisual=getLockedDay13ResumePresentation(state);
    immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};
    immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;
  }
  if(session.id===LOCKED_DAY14_SCENE_ID){const resumeVisual=getLockedDay14ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY15_SCENE_ID){const resumeVisual=getLockedDay15ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY16_SCENE_ID){const resumeVisual=getLockedDay16ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY17_SCENE_ID){const resumeVisual=getLockedDay17ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY18_SCENE_ID){const resumeVisual=getLockedDay18ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  ...
  if(session.id===LOCKED_DAY23_SCENE_ID){const resumeVisual=getDay23ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY24_SCENE_ID){const resumeVisual=getDay24ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY25_SCENE_ID){const resumeVisual=getDay25ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:state.storyFlags?.day25V4?resumeVisual.backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY26_SCENE_ID){const resumeVisual=getLockedDay26ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY27_SCENE_ID){const resumeVisual=getLockedDay27ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
  if(session.id===LOCKED_DAY28_SCENE_ID){const resumeVisual=getDay28ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:resumeVisual.backgroundUrl??getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??null;}
  if(session.id===LOCKED_DAY29_SCENE_ID){const resumeVisual=getDay29ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:resumeVisual.backgroundUrl??getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??null;}
  ...
  enforceModeExclusiveUi("story-start");
...
function updateImmersiveCharacter(expressionId="calm") {
  ...
  const npcSprite=characterId!=="girlfriend"?(immersiveScene?.id===LOCKED_DAY18_SCENE_ID&&immersiveScene.activeCharacterAssetUrl?immersiveScene.activeCharacterAssetUrl:getNpcSprite(characterId)):"";
...
function renderStoryFreeAction(step){
  ...
  const definition=state.day===8&&state.storyFlags?.day8V3Complete===true?{...baseDefinition,storySceneId:LOCKED_DAY8_SCENE_ID,description:"지훈의 현재 삶을 듣고 하은과 서로 다른 오후를 나눈 뒤다. 오늘 확인한 관계와 공개 경계 중 하나만 정리한다.",nextSchedule:"정리를 마치면 DAY 9, 하은이 보낸 두 가지 옷 색상 질문으로 넘어간다."}:state.day===9&&state.storyFlags?.day9V3Complete===true?{...baseDefinition,storySceneId:LOCKED_DAY9_SCENE_ID,description:"명동에서 서로의 옷과 선물 경계를 직접 고르고 돌아왔다. 오늘 만든 현재의 기억 중 하나만 정리한다.",nextSchedule:"정리를 마치면 DAY 10, 장보기 전에 연락하기로 한 약속으로 넘어간다."}:state.day===10&&state.storyFlags?.day10V3Complete===true?{...baseDefinition,storySceneId:LOCKED_DAY10_SCENE_ID,description:"직접 준비한 저녁에서 시간·배고픔·변경 합의와 서로의 독립 일정을 확인했다. 오늘 만든 관계 기록 중 하나만 정리한다.",nextSchedule:"정리를 마치면 DAY 11, 소라의 독립 약속과 두 사람의 동의 조건으로 넘어간다."}:baseDefinition;
  ...
  if(progress.status==="REPORT"&&progress.day===1){renderDay1HospitalNight();return;}
  state.pendingStoryId=definition.storySceneId;
  $("#skipButton").classList.add("hidden");
  $("#storyChoiceLayer").classList.add("hidden");
  $("#vnCharacter").hidden=true;$("#vnNpcRear").hidden=true;$("#vnNpcFront").hidden=true;$("#vnEventCg").hidden=true;
  $("#visualNovelStage").classList.add("story-free-action-active");
  const layer=$("#storyFreeActionLayer");layer.classList.remove("hidden");
  if(progress.status==="ACTIVE"){
    const actionRows=getStoryFreeActions(state).map(action=>`<button type="button" class="story-free-action-card" data-story-free-action="${escapeHtml(action.id)}"><span aria-hidden="true">${action.icon}</span><b>${escapeHtml(action.title)}</b><small>${escapeHtml(action.description)}</small></button>`).join("");
    const locks=STORY_FEATURES.map(feature=>{const availability=getStoryFeatureAvailability(state,feature.id);return `<div class="story-feature-lock${availability.available?" is-unlocked":""}" aria-label="${escapeHtml(feature.label)} ${availability.available?"사용 가능":"잠금"}"><b>${availability.available?"✓":"🔒"} ${escapeHtml(feature.label)}</b><small>${escapeHtml(availability.available?"현재 스토리 범위에서 사용할 수 있습니다.":availability.reason)}</small></div>`;}).join("");
  ...
  }else if(progress.status==="REPORT"){
    const report=getStoryFreeActionReport(state),changes=report.filter(item=>Number(item.delta)).map(item=>`<span>${escapeHtml(item.label)} <b>${item.delta>0?"+":""}${Math.round(item.delta)}</b></span>`).join("");
    const eventCopy=progress.event?`<aside class="story-feature-locks"><h3>공용 이벤트 완료</h3><b>${escapeHtml(progress.event.title??progress.event.id)}</b><small>${escapeHtml(progress.event.choices?.length?"기존 이벤트 장면에서 선택 결과가 저장되었습니다.":progress.event.text??"")}</small></aside>`:`<aside class="story-feature-locks"><h3>공용 이벤트</h3><small>현재 Context에서 이번에는 사건이 발생하지 않았습니다.</small></aside>`;
    layer.innerHTML=`<article class="story-free-action-shell"><header><span>DAY ${state.day} · FREE ACTION RESULT</span><h2>${escapeHtml(progress.result?.title??"자유행동 완료")}</h2><p>${escapeHtml(progress.result?.summary??"")}</p></header><section class="story-free-status">${changes||"<span>기록이 저장되었습니다.</span>"}</section>${eventCopy}<button id="completeStoryFreeAction" class="primary-button" type="button">SAVE · DAY ${state.day+1} →</button></article>`;
  }
  eventRuntime.input.unlock(immersiveScene?.id);
  enforceModeExclusiveUi("story-free-action");
  persistEventRuntime(true);SaveManager.save(state);
}

function launchStoryFreeActionEvent(progress){
  if(!progress?.event||progress.status!=="EVENT")return;
  closeStoryFreeAction();
  const owner=immersiveScene?.id;if(owner){eventRuntime.input.unlock(owner);eventRuntime.complete();immersiveScene=null;persistEventRuntime(true);}
  openEventScene(progress.event,{fromStoryFreeAction:true});
}

function closeStoryFreeAction(){
  $("#storyFreeActionLayer").classList.add("hidden");$("#storyFreeActionLayer").innerHTML="";
  $("#visualNovelStage").classList.remove("story-free-action-active");
}
...
function renderImmersiveStep() {
  ...
  if(immersiveScene?.id===LOCKED_DAY23_SCENE_ID&&isDay23V4ResolutionStep(step)){
    try{const response=getDay23V4RuntimeResolution(state,step);const {steps}=applyDay23V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 23의 현재 응답·결제 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY24_SCENE_ID&&isDay24V4ResolutionStep(step)){
    try{const response=getDay24V4RuntimeResolution(state,step);const {steps}=applyDay24V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 24의 현재 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY25_SCENE_ID&&isDay25V4ResolutionStep(step)){
    try{const response=getDay25V4RuntimeResolution(state,step);const {steps}=applyDay25V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 25의 현재 장소·관계·접촉·약속 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY26_SCENE_ID&&isDay26V4ResolutionStep(step)){
    try{const response=getDay26V4RuntimeResolution(state,step);const {steps}=applyDay26V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 26의 현재 참석·관계·정정·약속 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY27_SCENE_ID&&isDay27V4ResolutionStep(step)){
    try{const response=getDay27V4RuntimeResolution(state,step);const {steps}=applyDay27V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 27의 현재 대화·관계·거리·약속 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY28_SCENE_ID&&isDay28V4ResolutionStep(step)){
    try{const response=getDay28V4RuntimeResolution(state,step);const {steps}=applyDay28V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 28의 현재 만남·관계·접촉·집 초대 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY29_SCENE_ID&&isDay29V4ResolutionStep(step)){
    try{const response=getDay29V4RuntimeResolution(state,step);const {steps}=applyDay29V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 29의 현재 만남·관계·접촉·숙박 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  if(immersiveScene?.id===LOCKED_DAY30_SCENE_ID&&isDay30V4ResolutionStep(step)){
    try{const response=getDay30V4RuntimeResolution(state,step);const {steps}=applyDay30V4GameResolution(state,response);SaveManager.save(state);immersiveScene.sequence=steps;immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 30의 현재 만남·관계·약속·사진 응답 상태를 확인할 수 없어 진행을 멈췄습니다.");}return;
  }
  ...
  if(step.type==="chapterCompletionCue"){
    try{const sceneEnd=step.day===30?completeDay30V4GameChapter(state,step):step.day===29?completeDay29V4GameChapter(state,step):step.day===28?completeDay28V4GameChapter(state,step):step.day===27?completeDay27V4GameChapter(state,step):step.day===26?completeDay26V4GameChapter(state,step):step.day===25?completeDay25V4GameChapter(state,step):step.day===24?completeDay24V4GameChapter(state,step):step.day===23?completeDay23V4GameChapter(state,step):step.day===22?completeDay22V4GameChapter(state,step):step.day===21?completeDay21V4GameChapter(state,step):step.day===20?completeDay20V4GameChapter(state,step):step.day===19?completeDay19V4GameChapter(state,step):step.day===18?completeDay18V4GameChapter(state,step):step.day===17?completeDay17V4GameChapter(state,step):step.day===16?completeDay16V4GameChapter(state,step):completeDay15V4GameChapter(state,step);SaveManager.save(state);immersiveScene.sequence=[sceneEnd];immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast(`DAY ${step.day??""} 완료 상태를 확인할 수 없어 진행을 멈췄습니다.`);}
    return;
  }
  ...
  if(step.backgroundId||step.backgroundUrl){
  ...
  if(step.type==="phoneCallCue"){
    finishDialogueTyping();
    const ended=step.status==="ended",shifted=step.status==="grip-shift",stage=$("#visualNovelStage");
    setStoryMessagePresentation(ended?null:{type:"dialogue",device:"call",speaker:step.speaker});
    stage.dataset.callCue=ended?"ended":shifted?"grip-shift":"silence";
    stage.classList.remove("narration-mode","is-typing");
    $("#sceneTitle").textContent=ended?"통화 종료":step.speaker;
    $("#sceneTitle").classList.remove("hidden");
    $("#sceneText").textContent="";
    stage.querySelector(".vn-dialogue").setAttribute("aria-label",ended?"통화 종료":shifted?"통화 중 · 휴대전화를 다른 손으로 옮김":"통화 중 · 잠시 침묵");
    queueSceneStep(shifted?650:1200);
    return;
  }
  if(step.type==="roomActionCue"||step.type==="storyActionCue"||step.type==="finalFadeCue"){
    finishDialogueTyping();
    const stage=$("#visualNovelStage"),finalFade=step.type==="finalFadeCue";
    setStoryMessagePresentation(null);
    if(finalFade)stage.dataset.finalFade="active";else stage.dataset[step.type==="roomActionCue"?"roomAction":"storyAction"]=step.status;
    stage.classList.remove("narration-mode","is-typing");
    $("#sceneTitle").textContent="";$("#sceneTitle").classList.add("hidden");$("#sceneText").textContent="";
    stage.querySelector(".vn-dialogue").setAttribute("aria-label",step.actionLabel);
    queueSceneStep(step.duration??900);
    return;
  }
  ...
  if(step.type==="alarmAction"){
    const layer=$("#vnEventCg"),stage=$("#visualNovelStage");
    layer.src=step.source;layer.style.objectFit=step.fit??"contain";layer.hidden=false;
    stage.dataset.alarmAction="active";stage.setAttribute("aria-label",step.actionLabel??"눌러서 알람 끄기");
    $("#sceneTitle").textContent="08:00 알람";$("#sceneTitle").classList.remove("hidden");
    $("#sceneText").textContent=step.actionLabel??"눌러서 알람 끄기";stage.classList.remove("narration-mode","is-typing");
    sound.playCue(step.sfxId,{cooldownMs:0});
    if(!eventRuntime.waitForInput("dialogue",{sceneId:eventRuntime.active?.sceneId,dialogueIndex:immersiveScene.index-1})){sound.stopCue(step.sfxId);eventRuntime.fail(new Error("Unable to open alarm action"));}
    else persistEventRuntime();
    return;
  }
  if(step.type==="cgShow"){
  ...
  if (step.type === "choice") { if(!eventRuntime.waitForInput("choice",{sceneId:eventRuntime.active?.sceneId,dialogueIndex:immersiveScene.index-1})){eventRuntime.fail(new Error("Unable to enter story choice state"),{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);return;}persistEventRuntime(true);renderImmersiveChoices(step.options); return; }
  if(step.type==="choiceSelection"){queueSceneStep(0);return;}
  if (step.type === "freeAction") { renderStoryFreeAction(step); return; }
  if(isInternalStoryStep(step)||!isPlayerFacingStoryStep(step)){queueSceneStep(0);return;}
...
function renderImmersiveChoices(options=[]) {
  ...
  const day8Stage=state.storyFlags?.day8RuntimeStage??0;
  const day8Prompts={0:"외출 중 연락 계약을 어떻게 정할까?",1:"현재 생활에 맞는 세제를 어떻게 고를까?",2:"귀가 뒤 결과를 어떤 방식으로 공유할까?"};
  const day9Stage=state.storyFlags?.day9RuntimeStage??0;
  const day9Prompts={0:"90분의 적응 범위를 어디에 둘까?",1:"권한 밖의 급한 질문을 어떻게 처리할까?",2:"다음 방문을 위한 피드백 계약을 어떻게 남길까?"};
  const day10Stage=state.storyFlags?.day10RuntimeStage??0;
  const day10Prompts={0:"세 시간 근무 리듬을 어떻게 나눌까?",1:"현재 동료와의 점심을 어떻게 보낼까?",2:"오늘의 세 시간을 어떤 기준으로 남길까?"};
  const day11Stage=state.storyFlags?.day11RuntimeStage??0;
  const day11Prompts={0:"한 주 생활표의 첫 기준을 어디에 둘까?",1:"일정이 겹치면 어떤 규칙을 적용할까?",2:"하은과 생활표를 어디까지 공유할까?"};
  const day12Stage=state.storyFlags?.day12RuntimeStage??0;
  const day12Prompts={0:"현재 계정을 어떤 공식 범위에서 확인할까?",1:"생활비의 소유권을 어떻게 분류할까?",2:"기본 금융 기능을 어디까지 열까?"};
  const day13Stage=state.storyFlags?.day13RuntimeStage??0;
  const day13Prompts={0:"현재 가계 예산의 첫 기준을 어디에 둘까?",1:"공동 비용 부담을 어떻게 합의할까?",2:"공동 장부를 어디까지 함께 볼까?"};
  const day14Stage=state.storyFlags?.day14RuntimeStage??0;const day14Prompts={0:"오늘 어떤 소비 범위를 시험할까?",1:"결제 전에 어떤 안전 규칙을 적용할까?",2:"선물의 동의 범위를 어떻게 정할까?"};
  const day15Stage=state.storyFlags?.day15RuntimeStage??0;const day15Prompts={0:"오늘의 여가 선택권을 어떻게 나눌까?",1:"계획을 바꿔야 할 때 무엇을 고를까?",2:"데이트 기록의 공개 범위를 어떻게 정할까?"};
  const day16Stage=state.storyFlags?.day16RuntimeStage??0;const day16Prompts={0:"지훈과의 현재 연락 범위를 어떻게 열까?",1:"다음 만남의 경계를 어떻게 정할까?",2:"내 상태와 과거 기록의 공유 범위를 어떻게 정할까?"};
  const day17Stage=state.storyFlags?.day17RuntimeStage??0;const day17Prompts={0:"현재 건강 루틴의 공식 기준을 무엇으로 삼을까?",1:"반복 가능한 건강 루틴을 어떻게 만들까?",2:"의료 데이터와 돌봄의 공유 범위를 어떻게 정할까?"};
  const day18Stage=state.storyFlags?.day18RuntimeStage??0;const day18Prompts={0:"현재 집의 안전 동선을 어떻게 만들까?",1:"현재 생활 물건과 약을 어떻게 보관할까?",2:"비상 출입 권한을 어디까지 허용할까?"};
  const day19Stage=state.storyFlags?.day19RuntimeStage??0;const day19Prompts={0:"여행 사진 다음에 무엇을 먼저 볼까?",1:"둘의 여행을 어떤 범위로 고칠까?",2:"오늘의 생활과 다음 시간을 위해 무엇을 남길까?"};
  const day20Stage=state.storyFlags?.day20RuntimeStage??0;const day20Prompts={0:"오늘 집에서 무엇을 준비할까?",1:"아무것도 하지 않는 시간을 어떻게 보낼까?",2:"오늘 밤의 끝을 어디에 둘까?"};
  const day21Stage=state.storyFlags?.day21RuntimeStage??0;const day21Prompts={0:"오늘 아침을 어떻게 시작할까?",1:"하은이 살아온 기다림을 어떻게 들을까?",2:"오늘 직접 들은 말에서 무엇을 남길까?"};
  const day22Stage=state.storyFlags?.day22RuntimeStage??0;const day22Prompts={0:"예상과 달라진 여행에서 무엇을 고칠까?",1:"서로 다른 속도의 시간을 어떻게 보낼까?",2:"떠날 수 있는 두 사람이 다음을 어떻게 고를까?"};
  const day23Stage=state.storyFlags?.day23RuntimeStage??0;const day23Prompts={0:"돌아가기 전 무엇을 남길까?",1:"각자의 집으로 돌아온 마음을 어떻게 말할까?",2:"같은 주소가 아니어도 이어질 다음을 어떻게 고를까?"};
  const day24Stage=state.storyFlags?.day24RuntimeStage??0;const day24Prompts={0:"끝내지 못한 첫 문장을 어떻게 말할까?",1:"상대의 대답을 기다리며 무엇을 책임질까?",2:"마침표 다음의 빈칸을 어떻게 둘까?"};
  const day25Stage=state.storyFlags?.day25RuntimeStage??0;const day25Prompts={0:"좋아한다는 말 뒤에 어떤 생활을 붙일까?",1:"서로 다른 미래를 어떻게 함께 살펴볼까?",2:"지금 실제로 지킬 수 있는 다음은 무엇일까?"};
  const day26Stage=state.storyFlags?.day26RuntimeStage??0;const day26Prompts={0:"사람들 앞에서 각자의 말을 어떻게 지킬까?",1:"축하가 두 사람보다 빨라질 때 무엇을 할까?",2:"관계의 이름 뒤에 있는 목소리를 어떻게 들을까?"};
  const day27Stage=state.storyFlags?.day27RuntimeStage??0;const day27Prompts={0:"흐리거나 삼킨 말을 어떻게 꺼낼까?",1:"사과 뒤에 오는 상대의 답을 어떻게 들을까?",2:"지울 수 없는 말을 어떻게 바로잡을까?"};
  const day28Stage=state.storyFlags?.day28RuntimeStage??0;const day28Prompts={0:"오늘의 거리에서 다시 만난다는 뜻은 무엇일까?",1:"가까워지고 싶은 마음과 남은 서운함을 어떻게 말할까?",2:"다음 만남의 거리를 어떻게 고를까?"};
  const day29Stage=state.storyFlags?.day29RuntimeStage??0;const day29Prompts={0:"지난 물건 대신 오늘의 나를 어떻게 고를까?",1:"내일을 앞두고 꼭 하고 싶은 한 가지는 무엇일까?",2:"실제로 지킬 수 있는 다음 약속은 무엇일까?"};
  const day30Stage=state.storyFlags?.day30RuntimeStage??0;const day30Prompts={0:"특별한 날의 평범한 아침을 어떻게 시작할까?",1:"마지막으로 숨기지 않을 마음은 무엇일까?",2:"그다음 하루를 위해 무엇을 남길까?"};
  ...
  const visiblePrompt=day2Exploration?`방 안을 조사해보자. 조사 ${day2SearchCount} / 3 · 서로 다른 세 곳을 확인하면 계속할 수 있다.`:(immersiveScene?.currentStep?.prompt??prompt);
  layer.innerHTML=`<p class="choice-prompt">${visiblePrompt}</p>${options.map((option,index)=>`<button type="button" data-immersive-choice="${escapeHtml(option.id)}"><span>${exploration?"◇":String(index+1).padStart(2,"0")}</span><b>${escapeHtml(option.label)}</b></button>`).join("")}`;
  layer.classList.remove("hidden");
  enforceModeExclusiveUi(exploration?"story-exploration":"story-choice");
...
function chooseImmersiveOption(choiceId) {
  ...
  eventRuntime.input.unlock(immersiveScene.id);if(!eventRuntime.resumePlaying({sceneId:eventRuntime.active?.sceneId}))return;persistEventRuntime(true);
...
function applySkippedScenePresentation(choiceIndex){
  ...
  if(transition){const hasCharacter=Object.hasOwn(transition,"characterId");if(transition.characterAssetUrl)immersiveScene.activeCharacterAssetUrl=transition.characterAssetUrl;immersiveScene.presentation={...immersiveScene.presentation,backgroundId:transition.backgroundId??immersiveScene.presentation.backgroundId,backgroundUrl:transition.backgroundUrl??(transition.backgroundId?getBackgroundAsset(transition.backgroundId):immersiveScene.presentation.backgroundUrl),characterId:hasCharacter?transition.characterId:immersiveScene.presentation.characterId,characterAssetUrl:transition.characterAssetUrl??immersiveScene.presentation.characterAssetUrl,expressionId:hasCharacter?transition.expressionId:transition.expressionId??immersiveScene.presentation.expressionId,poseId:hasCharacter?transition.poseId:transition.poseId??immersiveScene.presentation.poseId,outfitId:transition.outfitId??immersiveScene.presentation.outfitId,weather:transition.weather??immersiveScene.presentation.weather,timeOfDay:transition.timeOfDay??immersiveScene.presentation.timeOfDay,storyClock:transition.storyClock??immersiveScene.presentation.storyClock};applyScenePresentation(immersiveScene.presentation);if(hasCharacter&&transition.characterId===null){$("#vnCharacter").hidden=true;$("#vnCharacterVideo").hidden=true;}}
...
function skipImmersiveScene(event) {
  ...
  const resolution=immersiveScene.sequence.find(step=>step.type==="haeunContactResolution"||step.type==="haeunStayResolution"||step.type==="contactConsentCue"||step.type==="lodgingConsentCue"||step.type==="travelConfirmationCue"||step.type==="photoConsentCue"||isDay23V4ResolutionStep(step)||isDay24V4ResolutionStep(step)||isDay25V4ResolutionStep(step)||isDay26V4ResolutionStep(step)||isDay27V4ResolutionStep(step)||isDay28V4ResolutionStep(step)||isDay29V4ResolutionStep(step)||isDay30V4ResolutionStep(step));
  ...
  if(completionCue){
    try{
      if(completionCue.day===30)completeDay30V4GameChapter(state,completionCue);
      else if(completionCue.day===29)completeDay29V4GameChapter(state,completionCue);
      else if(completionCue.day===28)completeDay28V4GameChapter(state,completionCue);
      else if(completionCue.day===27)completeDay27V4GameChapter(state,completionCue);
      else if(completionCue.day===26)completeDay26V4GameChapter(state,completionCue);
      else if(completionCue.day===25)completeDay25V4GameChapter(state,completionCue);
      else if(completionCue.day===24)completeDay24V4GameChapter(state,completionCue);
      else if(completionCue.day===23)completeDay23V4GameChapter(state,completionCue);
      else if(completionCue.day===22)completeDay22V4GameChapter(state,completionCue);
...
function advanceCampaignChapter(completedSession) {
  ...
  $("#phaseLabel").textContent=phases[state.phase].label;$("#clockLabel").textContent=phases[state.phase].time;
...
function finishImmersiveScene() {
  ...
  const completedSession=immersiveScene;
  if(completedSession?.id===LOCKED_DAY1_SCENE_ID&&state.storyFlags?.day1QuestionStrategy)state.pendingStoryId=null;
  if(completedSession?.id===LOCKED_DAY2_SCENE_ID&&state.storyFlags?.day2ContactStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY2_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY2_SCENE_ID,getLockedDay2LegacyChoice(state));state.storyFlags.day2RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY2_SCENE_ID&&state.storyFlags?.day2RuntimeComplete&&!state.storyFlags?.day2FreeActionComplete){const freeAction={type:"freeAction",id:"day2-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id==="m30-day3-discharge-phone"&&state.storyHistory?.some(record=>record.sceneId===completedSession.id)){state.storyFlags.day3RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id==="m30-day3-discharge-phone"&&state.storyFlags?.day3RuntimeComplete&&!state.storyFlags?.day3FreeActionComplete){const freeAction={type:"freeAction",id:"day3-discharge-room-day",phase:"day",location:"hospital",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY4_SCENE_ID&&state.storyFlags?.day4SharingStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY4_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY4_SCENE_ID,getLockedDay4LegacyChoice(state));state.storyFlags.day4RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY4_SCENE_ID&&state.storyFlags?.day4RuntimeComplete&&!state.storyFlags?.day4FreeActionComplete){const freeAction={type:"freeAction",id:"day4-home-night",phase:"night",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY5_SCENE_ID&&state.storyFlags?.day5ReturnStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY5_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY5_SCENE_ID,getLockedDay5LegacyChoice(state));state.storyFlags.day5RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY5_SCENE_ID&&state.storyFlags?.day5RuntimeComplete&&!state.storyFlags?.day5FreeActionComplete){const freeAction={type:"freeAction",id:"day5-office-evening",phase:"evening",location:"office",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY6_SCENE_ID&&(state.storyFlags?.day6DatePlan||state.storyFlags?.day6V3Complete)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY6_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY6_SCENE_ID,getLockedDay6LegacyChoice(state));state.storyFlags.day6RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY6_SCENE_ID&&state.storyFlags?.day6RuntimeComplete&&!state.storyFlags?.day6FreeActionComplete){const freeAction={type:"freeAction",id:"day6-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY7_SCENE_ID&&(state.storyFlags?.day7MemoryStrategy||state.storyFlags?.day7V3Complete)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY7_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY7_SCENE_ID,getLockedDay7LegacyChoice(state));state.storyFlags.day7RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY7_SCENE_ID&&state.storyFlags?.day7RuntimeComplete&&!state.storyFlags?.day7FreeActionComplete){const freeAction={type:"freeAction",id:"day7-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY8_SCENE_ID&&(state.storyFlags?.day8ShareStrategy||state.storyFlags?.day8V3Complete)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY8_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY8_SCENE_ID,getLockedDay8LegacyChoice(state));state.storyFlags.day8RuntimeComplete=true;state.pendingStoryId=null;}
  ...
  if(completedSession?.id===LOCKED_DAY9_SCENE_ID&&(state.storyFlags?.day9DebriefStrategy||state.storyFlags?.day9V3Complete)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY9_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY9_SCENE_ID,getLockedDay9LegacyChoice(state));state.storyFlags.day9RuntimeComplete=true;state.pendingStoryId=null;}
  ...
  if(completedSession?.id===LOCKED_DAY10_SCENE_ID&&(state.storyFlags?.day10DebriefStrategy||state.storyFlags?.day10V3Complete)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY10_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY10_SCENE_ID,getLockedDay10LegacyChoice(state));state.storyFlags.day10RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY10_SCENE_ID&&state.storyFlags?.day10RuntimeComplete&&!state.storyFlags?.day10FreeActionComplete){const freeAction={type:"freeAction",id:"day10-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY11_SCENE_ID&&(state.storyFlags?.day11ShareStrategy||state.storyFlags?.day11V3Complete)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY11_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY11_SCENE_ID,getLockedDay11LegacyChoice(state));state.storyFlags.day11RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY11_SCENE_ID&&state.storyFlags?.day11RuntimeComplete&&!state.storyFlags?.day11FreeActionComplete){const freeAction={type:"freeAction",id:"day11-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY12_SCENE_ID&&(state.storyFlags?.day12AccessStrategy||state.storyFlags?.day12V3Completed)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY12_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY12_SCENE_ID,getLockedDay12LegacyChoice(state));state.storyFlags.day12RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY12_SCENE_ID&&getDay12V3Compatibility(state).mode==="V1_LEGACY"&&state.storyFlags?.day12RuntimeComplete&&!state.storyFlags?.day12FreeActionComplete){const freeAction={type:"freeAction",id:"day12-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY13_SCENE_ID&&(state.storyFlags?.day13ReviewStrategy||state.storyFlags?.day13V3Completed)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY13_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY13_SCENE_ID,getLockedDay13LegacyChoice(state));state.storyFlags.day13RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY13_SCENE_ID&&getDay13V3Compatibility(state).mode==="V1_LEGACY"&&state.storyFlags?.day13RuntimeComplete&&!state.storyFlags?.day13FreeActionComplete){const freeAction={type:"freeAction",id:"day13-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY14_SCENE_ID&&(state.storyFlags?.day14ConsentStrategy||state.storyFlags?.day14V4Completed)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY14_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY14_SCENE_ID,getLockedDay14LegacyChoice(state));state.storyFlags.day14RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY14_SCENE_ID&&getDay14V4Compatibility(state).mode==="V1_LEGACY"&&state.storyFlags?.day14RuntimeComplete&&!state.storyFlags?.day14FreeActionComplete){const freeAction={type:"freeAction",id:"day14-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY15_SCENE_ID&&(state.storyFlags?.day15PrivacyStrategy||state.storyFlags?.day15V4Completed)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY15_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY15_SCENE_ID,getLockedDay15LegacyChoice(state));state.storyFlags.day15RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY15_SCENE_ID&&getDay15V4Compatibility(state).mode==="V1_LEGACY"&&state.storyFlags?.day15RuntimeComplete&&!state.storyFlags?.day15FreeActionComplete){const freeAction={type:"freeAction",id:"day15-cafe-evening",phase:"evening",location:"cafe",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY16_SCENE_ID&&(state.storyFlags?.day16SharingStrategy||state.storyFlags?.day16V4Completed)&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY16_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY16_SCENE_ID,getLockedDay16LegacyChoice(state));state.storyFlags.day16RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY16_SCENE_ID&&getDay16V4Compatibility(state).mode==="V1_LEGACY"&&state.storyFlags?.day16RuntimeComplete&&!state.storyFlags?.day16FreeActionComplete){const freeAction={type:"freeAction",id:"day16-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY17_SCENE_ID&&state.storyFlags?.day17DataStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY17_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY17_SCENE_ID,getLockedDay17LegacyChoice(state));state.storyFlags.day17RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY17_SCENE_ID&&getDay17V4Compatibility(state).mode==="V1_LEGACY"&&state.storyFlags?.day17RuntimeComplete&&!state.storyFlags?.day17FreeActionComplete){const freeAction={type:"freeAction",id:"day17-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY18_SCENE_ID&&state.storyFlags?.day18AccessStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY18_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY18_SCENE_ID,getLockedDay18LegacyChoice(state));state.storyFlags.day18RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY18_SCENE_ID&&!state.storyFlags?.day18V4&&state.storyFlags?.day18RuntimeComplete&&!state.storyFlags?.day18FreeActionComplete){const freeAction={type:"freeAction",id:"day18-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  ...
  if(completedSession?.id===LOCKED_DAY23_SCENE_ID&&!state.storyFlags?.day23V4&&state.storyFlags?.day23FutureStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY23_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY23_SCENE_ID,getLockedDay23LegacyChoice(state));state.storyFlags.day23RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY23_SCENE_ID&&!state.storyFlags?.day23V4&&state.storyFlags?.day23RuntimeComplete&&!state.storyFlags?.day23FreeActionComplete){const freeAction={type:"freeAction",id:"day23-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY24_SCENE_ID&&!state.storyFlags?.day24V4&&state.storyFlags?.day24ConsentStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY24_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY24_SCENE_ID,getLockedDay24LegacyChoice(state));state.storyFlags.day24RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY24_SCENE_ID&&!state.storyFlags?.day24V4&&state.storyFlags?.day24RuntimeComplete&&!state.storyFlags?.day24FreeActionComplete){const freeAction={type:"freeAction",id:"day24-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY25_SCENE_ID&&!state.storyFlags?.day25V4&&state.storyFlags?.day25GuestStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY25_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY25_SCENE_ID,getLockedDay25LegacyChoice(state));state.storyFlags.day25RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY25_SCENE_ID&&!state.storyFlags?.day25V4&&state.storyFlags?.day25RuntimeComplete&&!state.storyFlags?.day25FreeActionComplete){const freeAction={type:"freeAction",id:"day25-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY26_SCENE_ID&&!state.storyFlags?.day26V4&&state.storyFlags?.day26SignatureStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY26_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY26_SCENE_ID,getLockedDay26LegacyChoice(state));state.storyFlags.day26RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY26_SCENE_ID&&!state.storyFlags?.day26V4&&state.storyFlags?.day26RuntimeComplete&&!state.storyFlags?.day26FreeActionComplete){const freeAction={type:"freeAction",id:"day26-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY27_SCENE_ID&&!state.storyFlags?.day27V4&&state.storyFlags?.day27ExecutionStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY27_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY27_SCENE_ID,getLockedDay27LegacyChoice(state));state.storyFlags.day27RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY27_SCENE_ID&&!state.storyFlags?.day27V4&&state.storyFlags?.day27RuntimeComplete&&!state.storyFlags?.day27FreeActionComplete){const freeAction={type:"freeAction",id:"day27-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY28_SCENE_ID&&!state.storyFlags?.day28V4&&state.storyFlags?.day28AdaptStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY28_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY28_SCENE_ID,getLockedDay28LegacyChoice(state));state.storyFlags.day28RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY28_SCENE_ID&&!state.storyFlags?.day28V4&&state.storyFlags?.day28RuntimeComplete&&!state.storyFlags?.day28FreeActionComplete){const freeAction={type:"freeAction",id:"day28-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY29_SCENE_ID&&!state.storyFlags?.day29V4&&state.storyFlags?.day29PlanStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY29_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY29_SCENE_ID,getLockedDay29LegacyChoice(state));state.storyFlags.day29RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY29_SCENE_ID&&!state.storyFlags?.day29V4&&state.storyFlags?.day29RuntimeComplete&&!state.storyFlags?.day29FreeActionComplete){const freeAction={type:"freeAction",id:"day29-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  if(completedSession?.id===LOCKED_DAY30_SCENE_ID&&!state.storyFlags?.day30V4&&state.storyFlags?.day30DecisionStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY30_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY30_SCENE_ID,getLockedDay30LegacyChoice(state));state.storyFlags.day30RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY30_SCENE_ID&&!state.storyFlags?.day30V4&&state.storyFlags?.day30RuntimeComplete&&!state.storyFlags?.day30FreeActionComplete){const freeAction={type:"freeAction",id:"day30-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
  reconcileCompletedStoryFreeAction(state,completedSession?.id);
  ...
  $("#visualNovelStage").classList.remove("narration-mode","location-event-scene","image-text-only");delete $("#visualNovelStage").dataset.focusCharacter;delete $("#visualNovelStage").dataset.sceneEffect;delete $("#visualNovelStage").dataset.roomAction;delete $("#visualNovelStage").dataset.storyAction;delete $("#visualNovelStage").dataset.finalFade;delete $("#vnCharacter").dataset.day1Pose;$("#skipButton").classList.add("hidden");$("#storyChoiceLayer").classList.add("hidden");$("#vnNpcRear").hidden=true;$("#vnNpcFront").hidden=true;$("#vnEventCg").hidden=true;$("#actionGrid").classList.toggle("hidden",state.scenario?.enabled===true);$("#nextButton").classList.toggle("hidden",state.scenario?.enabled===true);
...
function beginOnboarding() {
  if(state)resetActiveRuntimeForLoad();
  $("#storyChoiceLayer")?.classList.add("hidden");$("#storyChoiceLayer").innerHTML="";$("#storyFreeActionLayer")?.classList.add("hidden");$("#storyFreeActionLayer").innerHTML="";$("#actionGrid")?.classList.add("hidden");$("#nextButton")?.classList.add("hidden");
...
function openStoryIntro() {
  ...
  introVideoIndex=0;
  $("#storyIntroComplete").classList.add("hidden");
  $("#introLoading").classList.remove("hidden");
  $("#introLoadingText").textContent="프롤로그 준비 중";
  ...
  $("#introPlaybackHint").textContent="영상을 끝까지 감상하면 게임을 시작할 수 있습니다.";
  video.play().catch(()=>{ $("#introLoadingText").textContent="프롤로그를 재생할 준비가 됐습니다.";$("#introPlayButton").classList.remove("hidden"); });
}
...
function playNextIntroVideo() {
  ...
  video.play().catch(()=>{ $("#introLoading").classList.remove("hidden");$("#introLoadingText").textContent="프롤로그를 재생할 준비가 됐습니다.";$("#introPlayButton").classList.remove("hidden"); });
}

function setIntroStartLayerVisible(visible) {
  const screen=$("#storyIntroScreen"),layer=$("#storyIntroComplete");
...
function unlockIntroStart(message="프롤로그가 끝났습니다. 이제 게임을 시작하세요.") { $("#introLoading").classList.add("hidden");setIntroStartLayerVisible(true);$("#introPlaybackHint").textContent=message;$("#introGameStartButton").disabled=false; }
function updateIntroStartAvailability() {
  const video=$("#introVideo"),isLast=introVideoIndex===activeIntroVideoPlaylist.length-1;
...
function finishOnboarding() { requestInitialFullscreen();stopIntroPlayback();state=onboarding.previewState;SaveManager.save(state);showGame(); }
...
function restoreCampaignChapterProgress() {
  if(state.scenario?.enabled!==true)return false;
  reconcileCompletedStoryFreeAction(state);
  if(state.pendingStoryId)return false;
  let changed=false;
  for(let guard=0;guard<30;guard+=1){
    const completed=(state.storyHistory??[]).find(record=>record.sceneId.startsWith(`m30-day${state.day}-`));
    if(!completed)break;
    if(state.day===1&&!state.storyFlags?.day1FreeActionComplete){
      beginStoryFreeAction(state,"day1-hospital-evening");
...
function applyScenePresentation(presentation) {
  if(immersiveScene?.id===(state.storyFlags?.day30V4?LOCKED_DAY30_SCENE_ID:state.storyFlags?.day29V4?LOCKED_DAY29_SCENE_ID:state.storyFlags?.day28V4?LOCKED_DAY28_SCENE_ID:state.storyFlags?.day27V4?LOCKED_DAY27_SCENE_ID:state.storyFlags?.day26V4?LOCKED_DAY26_SCENE_ID:state.storyFlags?.day25V4?LOCKED_DAY25_SCENE_ID:state.storyFlags?.day24V4?LOCKED_DAY24_SCENE_ID:state.storyFlags?.day23V4?LOCKED_DAY23_SCENE_ID:state.storyFlags?.day22V4?LOCKED_DAY22_SCENE_ID:state.storyFlags?.day21V4?LOCKED_DAY21_SCENE_ID:state.storyFlags?.day20V4?LOCKED_DAY20_SCENE_ID:state.storyFlags?.day19V4?LOCKED_DAY19_SCENE_ID:LOCKED_DAY18_SCENE_ID)&&presentation.storyClock){$("#clockLabel").textContent=presentation.storyClock;$("#phaseLabel").textContent=({morning:"아침","late-morning":"오전",noon:"낮",day:"낮",afternoon:"낮","late-afternoon":"늦은 오후",evening:"저녁",night:"밤"})[presentation.timeOfDay]??"";}
...
function hasLateNightSpecialEvent(location) {
  if(isYuriCafeEncounterAvailable(state,location))return true;
...
function openWorldLocation(locationId) {
  ...
  if(isWorldTurnEnded()){showWorldTurnEndedPopup();return;}
  const lateNightYuri=ensureNightState(state).minutes>=22*60&&isYuriCafeEncounterAvailable(state,location),displayLocation=lateNightYuri?{...location,name:"카페 모퉁이 앞",description:"영업이 끝난 카페 앞 모퉁이. 늦은 귀갓길의 사람들이 가끔 지나간다."}:location,locationName=getWorldLocationName(displayLocation),home=location.category==="home";
  ...
  $("#modalContent").innerHTML=`<span class="eyebrow">${escapeHtml(map.name)} · ARRIVAL</span><h2>${displayLocation.icon} ${escapeHtml(locationName)}</h2><p>${escapeHtml(displayLocation.description)}</p>${home?"":`<div class="venue-menu-preview"><small>이곳에서 할 수 있는 일</small><strong>${escapeHtml(lateNightYuri?"문 닫은 카페 앞에서 주변 둘러보기":getVenueMenu(location))}</strong></div>`}<p class="venue-visit-question">${escapeHtml(locationName)}에 방문하시겠습니까?</p><div class="venue-confirm-actions"><button id="visitLocationCancel" type="button">아니오</button><button id="visitLocationConfirm" class="primary-button" type="button">예${home?" · 귀가하기":" · 방문하기"}</button></div>`;openModal();
  $("#visitLocationCancel").addEventListener("click",closeModal);
  $("#visitLocationConfirm").addEventListener("click",()=>{if(home){closeModal();returnToNightHome();return;}const result=spendNightTime(state,20,`${displayLocation.name} 방문`);if(!result.ok){showWorldTurnEndedPopup();return;}discoverLocation(state.world,location.id,state.day);state.logs.push({time:`DAY ${state.day} · MAP`,text:`${map.name}의 ${displayLocation.name}에 방문했다.`});if(location.category==="girlfriend-home"&&getPendingLateNightInvitation(state)){completeLateNightInvitationVisit(map,location);return;}if(isYujinRooftopInvitationReady(state,location.id,ensureNightState(state).minutes)){SaveManager.save(state);openYujinRooftopMeeting(map,location);return;}const repeatEncounter=rollRepeatWorldEncounter(state,displayLocation,ensureNightState(state).minutes);if(repeatEncounter){SaveManager.save(state);openRepeatWorldEncounter(map,displayLocation,repeatEncounter);return;}const locationEventPool=ensureNightState(state).minutes>=22*60?SITUATION_EVENTS.filter(event=>event.npcId!=="girlfriend"):SITUATION_EVENTS;let locationEvent=rollLocationSituationEvent(state,location,Math.random,locationEventPool);if(lateNightYuri&&locationEvent?.id===YURI_REUNION_EVENT_ID){locationEvent=structuredClone(locationEvent);locationEvent.message="문 닫은 카페 앞 모퉁이에서 오래전 헤어진 유리가 내 이름을 불렀다.";const scene=locationEvent.scenes?.[0];if(scene){scene.timeOfDay="night";scene.lighting="night-neon";const turns=scene.dialogueTurns??[];if(turns[0])turns[0].text=locationEvent.message;const invitation=turns.find(turn=>turn.text?.includes("오늘 저녁에 둘이 만나서"));if(invitation)invitation.text="이대로 헤어지기 아쉬운데, 근처를 조금 걸으면서 마지막으로 이야기하면 안 될까?";}}SaveManager.save(state);if(locationEvent){closeModal();$(".play-panel").classList.remove("hidden");$("#nightHome").classList.add("hidden");$("#worldMap").classList.add("hidden");openEventScene(locationEvent);return;}openWorldEventLayer(map,displayLocation);});
}
...
async function chatReply(message){
  ...
  if(session.mode==="call"&&sound.enabled)speakWithElevenLabs(response.text);
...
$("#storyFreeActionLayer").addEventListener("click",event=>{event.stopPropagation();const actionButton=event.target.closest("[data-story-free-action]");if(actionButton){const result=resolveStoryFreeAction(state,actionButton.dataset.storyFreeAction);if(!result)return;sound.play("confirm");SaveManager.save(state);renderStoryFreeAction({id:result.windowId});return;}if(event.target.closest("#completeStoryFreeAction")&&completeStoryFreeAction(state)){SaveManager.save(state);closeStoryFreeAction();finishImmersiveScene();}});
...
$("#introVideo").addEventListener("loadstart",()=>{if($("#storyIntroComplete").classList.contains("hidden")){$("#introLoading").classList.remove("hidden");$("#introLoadingText").textContent="프롤로그 준비 중";$("#introPlayButton").classList.add("hidden");}});
$("#introVideo").addEventListener("waiting",()=>{if($("#storyIntroComplete").classList.contains("hidden")){$("#introLoading").classList.remove("hidden");$("#introLoadingText").textContent="프롤로그 불러오는 중";}});
$("#introVideo").addEventListener("playing",()=>$("#introLoading").classList.add("hidden"));
$("#introVideo").addEventListener("error",()=>{if($("#storyIntroComplete").classList.contains("hidden")){$("#introLoading").classList.remove("hidden");$("#introLoadingText").textContent="영상을 불러오지 못했습니다. 잠시 후 다시 시도하거나 건너뛰어 주세요.";$("#introPlayButton").classList.remove("hidden");}});
$("#introPlayButton").addEventListener("click",()=>{$("#introPlayButton").classList.add("hidden");$("#introLoadingText").textContent="프롤로그 준비 중";$("#introVideo").play().catch(()=>{$("#introLoadingText").textContent="재생하지 못했습니다. 다시 시도해 주세요.";$("#introPlayButton").classList.remove("hidden");});});
