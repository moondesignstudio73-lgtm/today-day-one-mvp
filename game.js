...
import {prepareDay18V4GameEntry, getDay18V4GameSegment, getDay18V4GameResumePresentation, applyDay18V4GameChoice, completeDay18V4GameChapter} from "./src/day18-v4-game-bridge.mjs?v=61";
...
import { applyDay23V4GameChoice, applyDay23V4GameResolution, completeDay23V4GameChapter, getDay23V4GameCompatibility, getDay23V4GameResumePresentation, getDay23V4GameSegment, isDay23V4ResolutionStep, prepareDay23V4GameEntry } from "./src/day23-v4-game-bridge.mjs?v=1";
...
import { getBackgroundAsset, getGiftVehicleAsset, getNpcSprite } from "./src/assets/asset-manifest.mjs?v=24";
...
function openStoryScene(scene) {
  ...
  const freeResume=isStoryFreeActionResume(state,scene.id)&&!(lockedDay23&&day23V4);
...
function skipImmersiveScene(event) {
  ...
  const resolution=immersiveScene.sequence.find(step=>step.type==="haeunContactResolution"||step.type==="haeunStayResolution"||step.type==="contactConsentCue"||step.type==="lodgingConsentCue"||step.type==="travelConfirmationCue"||step.type==="photoConsentCue"||isDay23V4ResolutionStep(step));
  ...
  if(completionCue){
    try{
      if(completionCue.day===23)completeDay23V4GameChapter(state,completionCue);
...
function finishImmersiveScene() {
  ...
  if(completedSession?.id===LOCKED_DAY24_SCENE_ID&&state.storyFlags?.day24RuntimeComplete&&!state.storyFlags?.day24FreeActionComplete){const freeAction={type:"freeAction",id:"day24-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
...
function applyScenePresentation(presentation) {
  if(immersiveScene?.id===(state.storyFlags?.day23V4?LOCKED_DAY23_SCENE_ID:state.storyFlags?.day22V4?LOCKED_DAY22_SCENE_ID:state.storyFlags?.day21V4?LOCKED_DAY21_SCENE_ID:state.storyFlags?.day20V4?LOCKED_DAY20_SCENE_ID:state.storyFlags?.day19V4?LOCKED_DAY19_SCENE_ID:LOCKED_DAY18_SCENE_ID)&&presentation.storyClock){$("#clockLabel").textContent=presentation.storyClock;$("#phaseLabel").textContent=({morning:"아침",noon:"낮",day:"낮",afternoon:"낮",evening:"저녁",night:"밤"})[presentation.timeOfDay]??"";}
