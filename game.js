...
import { LOCKED_DAY16_SCENE_ID, applyLockedDay16ChoiceState, getLockedDay16LegacyChoice, getLockedDay16ResumePresentation, getLockedDay16Segment } from "./src/day16-campaign-runtime.mjs?v=2";
...
function openStoryScene(scene) {
  ...
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:freeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:state.storyFreeAction.phase,location:state.storyFreeAction.location,maxActions:state.storyFreeAction.maxActions},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?(day6V3?getDay6V3ImmersiveSegment(state).steps:getLockedDay6Segment(state)):lockedDay7?(day7V3?getDay7V3ImmersiveSegment(state).steps:getLockedDay7Segment(state)):lockedDay8?(day8V3?getDay8V3ImmersiveSegment(state).steps:getLockedDay8Segment(state)):lockedDay9?(day9V3?getDay9V3ImmersiveSegment(state).steps:getLockedDay9Segment(state)):lockedDay10?getLockedDay10Segment(state):lockedDay11?getLockedDay11Segment(state):lockedDay12?getLockedDay12Segment(state):lockedDay13?getLockedDay13Segment(state):lockedDay14?getLockedDay14Segment(state):lockedDay15?getLockedDay15Segment(state):lockedDay16?getLockedDay16Segment(state):lockedDay17?getLockedDay17Segment(state):lockedDay18?getLockedDay18Segment(state):lockedDay19?getLockedDay19Segment(state):lockedDay20?getLockedDay20Segment(state):lockedDay21?getLockedDay21Segment(state):lockedDay22?getLockedDay22Segment(state):lockedDay23?getLockedDay23Segment(state):lockedDay24?getLockedDay24Segment(state):lockedDay25?getLockedDay25Segment(state):lockedDay26?getLockedDay26Segment(state):lockedDay27?getLockedDay27Segment(state):lockedDay28?getLockedDay28Segment(state):lockedDay29?getLockedDay29Segment(state):lockedDay30?getLockedDay30Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
    ...
    if(lockedDay16){const result=applyLockedDay16ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay16Segment(state,result.stage);}
...
function renderImmersiveStep() {
  ...
  if(step.type==="chapterCompletionCue"){
    try{const sceneEnd=completeDay15V4GameChapter(state,step);SaveManager.save(state);immersiveScene.sequence=[sceneEnd];immersiveScene.index=0;immersiveScene.currentStep=null;queueSceneStep(0);}catch(error){eventRuntime.fail(error,{sceneId:eventRuntime.active?.sceneId});persistEventRuntime(true);toast("DAY 15 완료 상태를 확인할 수 없어 진행을 멈췄습니다.");}
    return;
  }
...
function finishImmersiveScene() {
  ...
  if(completedSession?.id===LOCKED_DAY16_SCENE_ID&&state.storyFlags?.day16SharingStrategy&&!state.storyHistory?.some(record=>record.sceneId===LOCKED_DAY16_SCENE_ID)){resolveStoryChoice(state,LOCKED_DAY16_SCENE_ID,getLockedDay16LegacyChoice(state));state.storyFlags.day16RuntimeComplete=true;state.pendingStoryId=null;}
  if(completedSession?.id===LOCKED_DAY16_SCENE_ID&&state.storyFlags?.day16RuntimeComplete&&!state.storyFlags?.day16FreeActionComplete){const freeAction={type:"freeAction",id:"day16-home-evening",phase:"evening",location:"home",maxActions:1};immersiveScene.sequence=[freeAction,{type:"sceneEnd"}];immersiveScene.index=1;immersiveScene.currentStep=freeAction;renderStoryFreeAction(freeAction);return;}
