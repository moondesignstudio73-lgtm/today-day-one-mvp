...
import {prepareDay18V4GameEntry, getDay18V4GameSegment, getDay18V4GameResumePresentation, applyDay18V4GameChoice, completeDay18V4GameChapter} from "./src/day18-v4-game-bridge.mjs?v=50";
...
import { getBackgroundAsset, getGiftVehicleAsset, getNpcSprite } from "./src/assets/asset-manifest.mjs?v=23";
...
function openStoryScene(scene) {
  ...
  const day19Resume=lockedDay19?getLockedDay19ResumePresentation(state):null;
  const day20Resume=lockedDay20?getLockedDay20ResumePresentation(state):null;
  const day21Resume=lockedDay21?getLockedDay21ResumePresentation(state):null;
  ...
  startImmersiveScene({id:scene.id,type:"story",presentation:lockedPresentation,sequence:freeResume?[{type:"freeAction",id:state.storyFreeAction.windowId,phase:state.storyFreeAction.phase,location:state.storyFreeAction.location,maxActions:state.storyFreeAction.maxActions},{type:"sceneEnd"}]:lockedDay1?getLockedDay1Segment(state,lockedSegment):lockedDay2?getLockedDay2Segment(state):lockedDay4?getLockedDay4Segment(state):lockedDay5?getLockedDay5Segment(state):lockedDay6?(day6V3?getDay6V3ImmersiveSegment(state).steps:getLockedDay6Segment(state)):lockedDay7?(day7V3?getDay7V3ImmersiveSegment(state).steps:getLockedDay7Segment(state)):lockedDay8?(day8V3?getDay8V3ImmersiveSegment(state).steps:getLockedDay8Segment(state)):lockedDay9?(day9V3?getDay9V3ImmersiveSegment(state).steps:getLockedDay9Segment(state)):lockedDay10?getLockedDay10Segment(state):lockedDay11?getLockedDay11Segment(state):lockedDay12?getLockedDay12Segment(state):lockedDay13?getLockedDay13Segment(state):lockedDay14?getLockedDay14Segment(state):lockedDay15?getLockedDay15Segment(state):lockedDay16?getLockedDay16Segment(state):lockedDay17?(day17V4?getDay17V4GameSegment(state):getLockedDay17Segment(state)):lockedDay18?getLockedDay18Segment(state):lockedDay19?getLockedDay19Segment(state):lockedDay20?getLockedDay20Segment(state):lockedDay21?getLockedDay21Segment(state):lockedDay22?getLockedDay22Segment(state):lockedDay23?getLockedDay23Segment(state):lockedDay24?getLockedDay24Segment(state):lockedDay25?getLockedDay25Segment(state):lockedDay26?getLockedDay26Segment(state):lockedDay27?getLockedDay27Segment(state):lockedDay28?getLockedDay28Segment(state):lockedDay29?getLockedDay29Segment(state):lockedDay30?getLockedDay30Segment(state):createStorySceneSequence(scene,presentation,getAvailableStoryChoices(state,scene)),onChoice:choiceId=>{
    ...
    if(lockedDay19){const result=applyLockedDay19ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay19Segment(state,result.stage);}
    ...
    if(lockedDay21){const result=applyLockedDay21ChoiceState(state,choiceId);if(!result)return null;SaveManager.save(state);return getLockedDay21Segment(state,result.stage);}
...
function startImmersiveScene(session) {
  ...
  if(session.id===LOCKED_DAY21_SCENE_ID){const resumeVisual=getLockedDay21ResumePresentation(state);immersiveScene.presentation={...immersiveScene.presentation,...resumeVisual,backgroundUrl:getBackgroundAsset(resumeVisual.backgroundId)};immersiveScene.activeCharacterAssetUrl=resumeVisual.characterAssetUrl??immersiveScene.activeCharacterAssetUrl;}
...
function renderImmersiveStep() {
  ...
  if(step.type==="roomActionCue"||step.type==="finalFadeCue"){
    finishDialogueTyping();
    const stage=$("#visualNovelStage"),finalFade=step.type==="finalFadeCue";
    setStoryMessagePresentation(null);
    if(finalFade)stage.dataset.finalFade="active";else stage.dataset.roomAction=step.status;
    stage.classList.remove("narration-mode","is-typing");
...
function applyScenePresentation(presentation) {
  if(immersiveScene?.id===LOCKED_DAY18_SCENE_ID&&state.storyFlags?.day18V4&&presentation.storyClock){$("#clockLabel").textContent=presentation.storyClock;$("#phaseLabel").textContent=({morning:"아침",afternoon:"낮",evening:"저녁",night:"밤"})[presentation.timeOfDay]??"";}
