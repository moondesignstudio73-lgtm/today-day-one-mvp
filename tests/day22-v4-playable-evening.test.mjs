...
test('separate Busan rooms expose no face praise, physical contact, or shared bed memory',()=>{
  const state=completedDay21ForDay22('BUSAN_TRIP',{shared:false});reachEvening(state);chooseDay22(state);let steps=getDay22V4PlayableEvening(state.storyFlags.day22V4);assert.equal(steps.some(step=>step.text==='하은이 씻고 편한 차림으로 나왔다.'),false);assert.equal(steps.at(-1).options.some(option=>option.id.endsWith('_like_relaxed_face')),false);assertSourced(steps);chooseDay22(state);steps=getDay22V4PlayableEvening(state.storyFlags.day22V4);assert.equal(steps.some(step=>step.text?.includes('침대 가장자리')),false);chooseDay22(state);steps=getDay22V4PlayableEvening(state.storyFlags.day22V4);assert.equal(steps.at(-1).options.length,1);assert.ok(steps.at(-1).options[0].id.endsWith('_rest_as_is'));assert.equal(steps.some(step=>step.text==='하은이 내 쪽으로 손을 내밀었다.'),false);assertSourced(steps);
...
test('Seoul day uses no lodging scene and returns through SCENE22 without SCENE21',()=>{
  const state=completedDay21ForDay22('SEOUL_DAY');reachEvening(state);let steps=getDay22V4PlayableEvening(state.storyFlags.day22V4);assert.equal(steps[0].location,'seoul-evening');assert.ok(steps.some(step=>step.text?.includes('숙소에 들어가는 화면은 없었다')));assert.equal(steps.some(step=>step.text?.includes('실제 예약한 곳')),false);assertSourced(steps);
  for(let i=0;i<6;i++){chooseDay22(state);if(i<5){steps=getDay22V4PlayableEvening(state.storyFlags.day22V4);assert.equal(steps.some(step=>step.text?.startsWith('숙소에 도착했다고')),false);assertSourced(steps);}}
