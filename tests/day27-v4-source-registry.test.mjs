...
test('DAY27 source registry preserves 24 scenes and every authored choice block',()=>{assert.deepEqual(DAY27_V4_SOURCE_SCENES.map(scene=>scene.number),Array.from({length:24},(_,index)=>index+1));assert.equal(choices().length,23);assert.equal(choices().every(choice=>choice.labels.length>=3&&choice.labels.length<=4),true);const expected={COMMON_MORNING:[1],CONVERSATION_START:[2],RELATIONSHIP_TRUTH:[3,4,5,6,7],HONEST_LISTENING:[8,9,10],PUBLIC_CORRECTION:[11],CONVERSATION_END:[12],JIHOON_SUPPORT:[13],COMMON_EVENING:[14],CONTINUING_NIGHT:[15,16],SEPARATION_NIGHT:[15],NO_CONVERSATION:[3,4,5,6,7,8]};for(const [variant,numbers] of Object.entries(expected))assert.deepEqual(choices().filter(choice=>choice.variant===variant).map(choice=>choice.number),numbers);});
