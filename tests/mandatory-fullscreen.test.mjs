...
assert.match(source,/function loadGame\(\) \{ requestInitialFullscreen\(\);/);
assert.match(source,/document\.addEventListener\("pointerdown",\(\)=>\{if\(isGameplayVisible\(\)&&!document\.fullscreenElement\)requestInitialFullscreen\(\);\},true\);/);
