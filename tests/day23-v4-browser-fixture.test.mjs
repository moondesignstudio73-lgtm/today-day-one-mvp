...
test('DAY23 browser fixture builds verified DAY21-22 routes and preserves the user save',()=>{
  ...
  for(const route of ['busan-shared','busan-separate','seoul-day','seoul-yuri','seoul-seojin','seoul-ara','no-travel-contact','no-travel-unavailable','busan-rejections'])assert.match(source,new RegExp(`data-route="${route}"`));
  assert.match(source,/pendingContacts:recipient\?\[recipient\]:\[\]/);
