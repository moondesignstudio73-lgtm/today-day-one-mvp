...
test('DAY24 mobile harness seeds four replay-locked routes without SKIP and preserves saves',()=>{const source=readFileSync(new URL('./day24-v4-mobile-entry.html',import.meta.url),'utf8');for(const route of ['friendly','neutral','distant','mixed'])assert.match(source,new RegExp(`data-route="${route}"`));for(const marker of ['completedDay23ForDay24','conversationMethod','pendingContacts','day24-v4-mobile-qa-save-backup','테스트 전 저장을 복원했습니다'])assert.match(source,new RegExp(marker));assert.doesNotMatch(source,/skipButton|\.click\([^)]*SKIP/i);});
