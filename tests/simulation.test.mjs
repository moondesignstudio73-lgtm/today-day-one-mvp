...
import { getSituationEventState, resolveSituationEventChoice, validateSituationEventState } from "../src/situation-event-manager.mjs";
...
import { getEligibleStoryScenes, getStoryScene, resolveStoryChoice, selectNextStoryScene, validateStoryState } from "../src/story-manager.mjs";
...
assert.equal(SITUATION_EVENTS.length,40);
assert.ok(SITUATION_EVENTS.every(event=>event.scenes.length>=3));
assert.ok(SITUATION_EVENTS.every(event=>event.scenes.reduce((sum,scene)=>sum+scene.dialogueTurns.length,0)>=30));
assert.ok(SITUATION_EVENTS.every(event=>event.choices.length===3));
...
assert.deepEqual(Object.fromEntries(["romance","temptation","conflict","work","friends","money","travel","mystery"].map(category=>[category,SITUATION_EVENTS.filter(event=>event.category===category).length])),{romance:6,temptation:5,conflict:5,work:4,friends:3,money:2,travel:3,mystery:2});
...
console.log("✓ 상황 에피소드 40개 · 150 Scene · 1,260 Dialogue Turn · 선택/기억/저장 검증 통과");
...
assert.equal(STORY_SCENES.length,135);
...
const legacyEvents=EVENT_DEFINITIONS.filter(event=>event.kind==="micro").map(({kind,...event})=>event);
...
assert.equal(Object.values(ACTIONS).flat().length, 19);
assert.equal(new Set(Object.values(ACTIONS).flat().map(action => action.id)).size, 19);
console.log("✓ 4개 시간대·19개 행동 데이터 스키마 검증 통과");
...
assert.equal(GIRLFRIEND_JOBS.length,6);
assert.ok(GIRLFRIEND_JOBS.every(validateGirlfriendCareer));
assert.deepEqual(new Set(GIRLFRIEND_JOBS.map(career=>career.heroineId)),new Set(HEROINE_PROFILES.map(profile=>profile.id)));
for(const profile of HEROINE_PROFILES){const career=createGirlfriendCareer(profile.id);assert.equal(career.name,profile.job);assert.ok(career.incomeRange[1]>=career.incomeRange[0]);}
...
assert.equal(HEROINE_OUTFITS.length,60);
assert.equal(HEROINE_STORY_SCENES.length,100);
assert.ok(HEROINE_PROFILES.every(profile=>getHeroineOutfits(profile.id).length===10));
...
const eventAudit=auditEventSystems({storyScenes:STORY_SCENES,events:EVENT_DEFINITIONS,situationEvents:SITUATION_EVENTS});
...
assert.equal(eventAudit.rows.length,STORY_SCENES.length+EVENT_DEFINITIONS.length);
