...
import { getEligibleEvents, getEventDiagnostics, getEventProbability, MAX_EVENTS_PER_DAY, meetsConditions, rollEvent, triggerEvent } from "../src/event-manager.mjs";
import { EVENT_DEFINITIONS, validateEventData } from "../src/events-data.mjs";
import { SITUATION_EVENTS, validateSituationEvents } from "../src/situation-events-data.mjs";
import { getSituationEventState, resolveSituationEventChoice, rollLocationSituationEvent, validateSituationEventState } from "../src/situation-event-manager.mjs";
...
assert.deepEqual(getVisibleActionEffects({npcInterest:12,npcTrust:8,trust:-20,conflict:12}),[
  {key:"npcInterest",label:"상대 관심도",value:12},
  {key:"npcTrust",label:"상대 신뢰",value:8},
  {key:"trust",label:"신뢰도",value:-20},
...
assert.equal(SITUATION_EVENTS.length,44);
...
assert.deepEqual(Object.fromEntries(["romance","school-romance","temptation","conflict","work","friends","money","travel","mystery"].map(category=>[category,SITUATION_EVENTS.filter(event=>event.category===category).length])),{romance:8,"school-romance":10,temptation:5,conflict:5,work:4,friends:5,money:2,travel:3,mystery:2});
...
storyState.day = 2;
assert.equal(selectNextStoryScene(storyState).id,"unread-message");
storyState.pendingStoryId = "unread-message";
const unreadResult = resolveStoryChoice(storyState,"unread-message","ignore");
assert.equal(unreadResult.record.choiceId,"ignore");
assert.equal(storyState.storyFlags.ignoredHardDay,true);
assert.equal(storyState.pendingStoryId,null);
assert.ok(storyState.memories.some(memory=>memory.tags.includes("unread-message")));
storyState.day = 6;
assert.ok(getEligibleStoryScenes(storyState).some(scene=>scene.id==="unread-followup"));
const followup = resolveStoryChoice(storyState,"unread-followup","apologize");
assert.match(followup.response,/다음에는/);
assert.equal(validateStoryState(storyState),true);
const contactScene = getStoryScene("contact-boundary");
const contactHigh = createInitialState(generateGirlfriend(() => 0.5),() => 0.5);
contactHigh.day=4;contactHigh.partner.personality.contactImportance=90;contactHigh.partner.personality.independence=20;
const highContactAffection=contactHigh.affection;
resolveStoryChoice(contactHigh,contactScene.id,"frequent");
assert.ok(contactHigh.affection>highContactAffection);
const contactIndependent = createInitialState(generateGirlfriend(() => 0.5),() => 0.5);
contactIndependent.day=4;contactIndependent.partner.personality.contactImportance=20;contactIndependent.partner.personality.independence=90;
const independentAffection=contactIndependent.affection;
resolveStoryChoice(contactIndependent,contactScene.id,"frequent");
assert.ok(contactIndependent.affection<independentAffection);
...
assert.equal(selectNextStoryScene(hiddenStoryState).id,"hidden-perfect-first-day");
const initialBoundary=hiddenStoryState.hiddenRoute.boundary;
resolveStoryChoice(hiddenStoryState,"hidden-perfect-first-day","set-pace");
...
for (const scene of HIDDEN_ROUTE_SCENES) {
  ...
  const selectedHiddenScene=selectNextStoryScene(hiddenRouteRun);
  assert.equal(selectedHiddenScene?.id,scene.id,`히든 루트 장면 순서: ${scene.id}`);
  const balancedChoice=scene.choices[1] ?? scene.choices[0];
  assert.ok(resolveStoryChoice(hiddenRouteRun,scene.id,balancedChoice.id));
}
