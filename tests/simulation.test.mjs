...
assert.equal(STORY_SCENES.length,35);
...
assert.equal(Object.values(ACTIONS).flat().length, 17);
assert.equal(new Set(Object.values(ACTIONS).flat().map(action => action.id)).size, 17);
console.log("✓ 4개 시간대·17개 행동 데이터 스키마 검증 통과");
