...
assert.equal(SITUATION_EVENTS.length,43);
...
assert.deepEqual(Object.fromEntries(["romance","school-romance","temptation","conflict","work","friends","money","travel","mystery"].map(category=>[category,SITUATION_EVENTS.filter(event=>event.category===category).length])),{romance:9,"school-romance":10,temptation:5,conflict:5,work:4,friends:3,money:2,travel:3,mystery:2});
...
assert.equal(STORY_SCENES.length,138);
assert.equal(MARRIAGE_30_STORY_SCENES.length,3);
...
assert.equal(hospitalSequence.filter(step=>step.type==="dialogue").length,4);
const hospitalResult=resolveStoryChoice(campaignStoryState,hospitalScene.id,"verify-medical");
assert.equal(hospitalResult.record.day,1);
assert.equal(campaignStoryState.scenario.investigation,8);
...
assert.equal(createStorySceneSequence(rehabilitationScene,resolveStoryPresentation(rehabilitationScene,campaignStoryState)).filter(step=>step.type==="dialogue").length,4);
const confidenceBeforeRehabilitation=campaignStoryState.confidence;
resolveStoryChoice(campaignStoryState,rehabilitationScene.id,"use-the-rail");
...
assert.equal(campaignStoryState.scenario.investigation,21);
