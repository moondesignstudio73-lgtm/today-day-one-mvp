...
import { DAY9_HOME_ACTIONS, DAY9_V3_HOME_ACTIONS, beginStoryFreeAction, completeStoryFreeAction, getStoryFeatureAvailability, getStoryFreeActions, markStoryFreeActionEventComplete, resolveStoryFreeAction } from "../src/story-free-action-manager.mjs";
...
assert.equal(FREE_MODE_EVENT_CATALOG.length,64);assert.equal(DAY9_HOME_ACTIONS.length,5);
...
const v3=create();v3.storyFlags.day9V3Complete=true;assert.equal(DAY9_V3_HOME_ACTIONS.length,5);assert.deepEqual(getStoryFreeActions(v3).map(action=>action.id),DAY9_V3_HOME_ACTIONS.map(action=>action.id));assert.equal(getStoryFreeActions(v3).some(action=>/직장|서진|동료|근무/.test(`${action.title} ${action.description}`)),false);assert.equal(getCompatibleSharedEvents(v3,context).some(event=>event.id==="context-day9-home-separate-feedback"),false);assert.match(getStoryFreeActions(v3).find(action=>action.id==="prepare-day10-contact-note").description,/장보기 전에 연락/);
...
const gameSource=readFileSync(new URL("../game.js",import.meta.url),"utf8");assert.match(gameSource,/completedSession\?\.id===LOCKED_DAY9_SCENE_ID[\s\S]*?day9FreeActionComplete/);assert.match(gameSource,/fromStoryFreeAction/);assert.match(gameSource,/getSharedEventById\(saved\.activeEvent\)/);assert.match(gameSource,/명동에서 서로의 옷과 선물 경계/);assert.match(gameSource,/DAY 10, 장보기 전에 연락하기로 한 약속/);
