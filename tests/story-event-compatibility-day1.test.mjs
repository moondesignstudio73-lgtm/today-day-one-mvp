...
const compatibleStateFor=event=>{
  ...
  const probe=evaluateEventCompatibility(state,event,{phoneUnlocked:true,financeUnlocked:true,npcIntroduced:event.npcRequirements??[]});
  return {state,context:{occurrence:"free-action-result",location:probe.allowedLocations[0]??"home",phase:probe.allowedPhases[0]??"night",phoneUnlocked:true,financeUnlocked:true,npcIntroduced:event.npcRequirements??[]}};
};

assert.equal(FREE_MODE_EVENT_CATALOG.length,63);
assert.equal(SHARED_EVENT_CATALOG.length,66);
assert.equal(new Set(SHARED_EVENT_CATALOG.map(event=>event.id)).size,66);
for(const event of FREE_MODE_EVENT_CATALOG){const {state,context}=compatibleStateFor(event);assert.equal(evaluateEventCompatibility(state,event,context).eligible,true,event.id);}
...
assert.deepEqual(candidates.map(event=>event.id).sort(),CONTEXTUAL_SHARED_EVENTS.map(event=>event.id).sort());
...
assert.equal(day1.eventHistory.filter(record=>record.category==="story-free-action").length,1);
