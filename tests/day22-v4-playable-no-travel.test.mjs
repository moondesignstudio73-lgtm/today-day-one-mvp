...
test('solo NO_TRAVEL plays C3-8 without Haeun, tickets, Busan food, or lodging',()=>{
  ...
  const rendered=all.map(step=>step.text??'').join('\n');assert.doesNotMatch(rendered,/표를 샀|밀면을 먹|숙소에|하은이 기다/);assert.equal(all.some(step=>step.speaker==='하은'||step.sender==='하은'),false);assert.equal(state.storyFlags.day22V4.facts.noTravelContact,null);
});
...
test('contact-available rest route exposes C6 and only recalls Haeun after that choice',()=>{
  ...
  steps=getDay22V4PlayableNoTravel(state.storyFlags.day22V4);assert.equal(steps.at(-1).choiceNumber,6);assert.equal(steps.some(step=>step.speaker==='하은'),false);assertSourced(steps);chooseDay22(state,'ask_haeun_day');
