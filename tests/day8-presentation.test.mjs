...
for(const [id,scene] of Object.entries(DAY8_PRESENTATION_SCENES)){
  for(const backgroundId of [scene.backgroundId,...Object.values(scene.branchBackgrounds??{})]){
    if(DAY8_REQUIRED_BACKGROUND_ASSETS[backgroundId]) continue;
...
assert.equal(BACKGROUND_ASSETS["day8-household-store-day"],undefined,"pending art is not registered as ready");
assert.equal(existsSync(new URL(`../${DAY8_REQUIRED_BACKGROUND_ASSETS["day8-household-store-day"]}`,import.meta.url)),false,"new art remains next gate");
...
console.log("✓ DAY 8 기존 배경·단독 구간·분기·신규 생활용품점·BGM/SFX 계약 검증 통과");
