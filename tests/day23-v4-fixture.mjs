...
export function completedDay22ForDay23(route,options={}){
  ...
  while(state.storyFlags.day22V4.phase!=='ending'){
    ...
    const selected=suffix?available.find(option=>option.id.endsWith(`_${suffix}`)):available[0];assert.ok(selected,`DAY22 ${chapter.phase}`);applyDay22V4Choice(state,selected.id);
  }
  completeDay22V4(state,{type:'chapterCompletionCue',day:22,finalSceneReached:true});
  return state;
}

export function chooseDay23(state,suffix){const options=getDay23V4Options(state.storyFlags.day23V4);const selected=suffix?options.find(option=>option.id.endsWith(`_${suffix}`)):options[0];assert.ok(selected,`DAY23 ${state.storyFlags.day23V4.phase}:${suffix}`);return applyDay23V4Choice(state,selected.id);}
