...
test('DAY22 browser fixture replays production history and preserves the user save',()=>{
  ...
  for(const route of ['busan-shared','busan-separate','seoul-day'])assert.match(source,new RegExp(`data-route="${route}"`));
  for(const suffix of ['shared_room_wish','separate_spaces','seoul'])assert.match(source,new RegExp(suffix));
