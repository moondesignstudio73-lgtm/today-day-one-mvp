...
for(const [id,scene] of Object.entries(DAY9_PRESENTATION_SCENES)){
  ...
  if(scene.characterId===null)assert.equal(scene.expressionId,null,`${id} protagonist solo`);
  else assert.ok(scene.characterId==="girlfriend"?getCharacterSprite(scene.characterId,scene.expressionId,scene.poseId):getNpcSprite(scene.characterId),`${id} character`);
...
assert.equal(Object.values(DAY9_PRESENTATION_SCENES).filter(scene=>scene.backgroundId==="day9-office-project-room-day").length,3);
