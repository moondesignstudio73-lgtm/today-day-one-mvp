...
export const ENDING_DEFINITIONS = [
  ...
  { id:"love-breakup", title:"사랑하지만 이별", description:"좋아하는 마음은 남았지만 함께 살아갈 방법을 끝내 찾지 못했다.", matches:state => state.affection < 350 || state.trust < 250 },
  ...
  { id:"wealthy-marriage", title:"경제적으로 성공한 결혼", description:"사랑과 경제적 안정을 함께 쌓은 두 사람은 든든한 미래를 약속했다.", matches:state => getAssetSummary(state).netWorth >= ENDING_BALANCE_THRESHOLDS.wealthyNetWorth && state.affection >= 650 && state.trust >= 600 },
  { id:"happy-marriage", title:"행복한 결혼", description:"수많은 선택 끝에 서로를 가장 잘 아는 두 사람은 평생을 약속했다.", matches:state => state.affection >= 850 && state.trust >= 800 && state.conflict < 45 },
  { id:"love-marriage", title:"사랑으로 결혼", description:"완벽하지 않아도 서로의 편이 되어 온 시간은 한 번뿐인 약속이 되었다.", matches:state => state.affection >= ENDING_BALANCE_THRESHOLDS.loveAffection && state.trust >= ENDING_BALANCE_THRESHOLDS.loveTrust },
  { id:"marriage-postponed", title:"결혼 연기", description:"사랑은 충분하지만 지금은 각자의 성장을 조금 더 기다리기로 했다.", matches:state => state.affection >= 550 && state.trust >= 550 && state.partner.personality.marriageDesire < 45 },
