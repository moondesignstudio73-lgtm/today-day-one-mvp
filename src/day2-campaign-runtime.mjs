...
function segment4(state){const out=[];if(state.storyFlags?.day2PendingPhotoReaction){out.push(...photoReaction(state.storyFlags.day2PendingPhotoReaction),clearPov(),tr("SCENE 09 · 내 방",BG.room),sprite("doorframe-permission-wait"),d("나","어느 문인지 전혀 모르겠어."),d("하은","저기. 열어 볼지는 네가 정하고."),n("침대, 책상, 옷장, 전원이 빠진 컴퓨터, 벽의 사진이 차례로 드러났다."),d("나","생각보다 평범하네."),d("하은","비밀 연구실이라도 기대했어?","warm-playful"),d("나","조금."),d("하은","컴퓨터 앞에서 게임하다 밤새는 연구원이면 비슷하긴 하다."),d("나","나 게임 많이 했어?"),d("하은","조금."),d("나","조금?"),d("하은","많이.","warm-playful"),n("하은은 문 옆에 서서 안으로 먼저 들어오지 않았다."));}if(state.storyFlags?.day2LastSearch)out.push(...(SEARCH_REACTIONS[state.storyFlags.day2LastSearch]??[]));out.push(pov("search-interactions"),searchChoice(state));return out;}
...
function segment5(state){const last=state.storyFlags?.day2LastSearch;const key=state.storyFlags?.day2KeyStrategy;return [
  ...(key?keyReaction(key):(SEARCH_REACTIONS[last]??[])),clearPov(),n("세 번째 탐색을 마쳤다. 나머지는 피로가 가신 뒤 다시 확인하기로 했다."),
