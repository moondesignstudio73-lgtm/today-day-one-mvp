...
function scene18(chapter){
  ...
  if(planned)steps.push(grounded(18,'하은과 통화하기로 한 시간을 확인했다.','하은과 통화하기로 한 날이면 시간을 확인했다.'),quoted(18,'**주인공** “지금 통화 괜찮아?”'),quoted(18,'**하은** “응. 나 밥 먹고 있어.”'),quoted(18,'**주인공** “그럼 다 먹고 할까?”'),quoted(18,'**하은** “짧게는 괜찮아. 너도 먹어.”'));
...
function reaction15(chapter){
  ...
  return chapter.input.contactAllowed?[quoted(18,'**하은** “좋아. 잘 먹어.”'),quoted(18,'**주인공** “너도.”'),mono(18,'짧은 인사 뒤에 음식에 집중했다. 통화가 짧다고 마음까지 짧아지는 것은 아니었다.')]:[grounded(18,'나는 내 저녁을 혼자 마치고 그릇을 정리했다.','하은과 연락을 쉬는 날이면 나는 내 저녁을 혼자 마쳤다. 통화하지 않은 상대의 목소리를 추억처럼 쓰지 않았다.')];
}
...
function scene19(chapter){
  ...
  else steps.push(grounded(19,'하은은 오늘 못 봤으니 더 궁금한 거냐고 웃었다.','오늘 실제로 만나지 않았다면 그녀는 “오늘은 못 봤으니까”라고 말하거나 웃었다. 아까까지 함께 있었다는 대사가 따라오지는 않았다.'));
...
function scene20(chapter){
  ...
  else steps.push(grounded(20,'나는 내일 이야기하고 싶은 것이 있다고 말했다.','하은이 통화할 수 있다면 나는 내일 이야기하고 싶은 것이 있다고 말했다.'),quoted(20,'**주인공** “좋았던 시간은 좋았어. 그런데 그 말로 미뤄 둔 대답까지 덮고 싶지는 않아.”'),mono(20,'하은은 조용히 들었다.'),quoted(20,'**하은** “내일 무슨 답을 들을지 미리 겁날 것 같아.”'),quoted(20,'**주인공** “그럴 것 같아.”'),quoted(20,'**하은** “그래도 아무 얘기 없다고 해 놓고 갑자기 말하는 것보다는 나아.”'),mono(20,'나는 그녀가 바로 고마워할 것이라고 기대하지 않았다.'),mono(20,'내 마음이 흔들리거나 끝내고 싶다는 쪽이면, 좋아한다는 말을 덧붙여 상황을 부드럽게 만들려 하지 않았다. 말할 수 있는 만큼만 말했다.'));
...
function scene22(chapter){
  ...
  if(pending)steps.push(grounded(22,'설명이 남은 대화창을 열었다. 내가 애매하게 남긴 문장이 있었다.','다른 사람과의 연락을 흐리게 남긴 경로라면 나는 그 창을 열었다.'),grounded(22,'그 문장을 고쳐 말할지, 아직 못 하겠다고 솔직히 미룰지 생각했다.','아직 하은과 만나면서 혼자라고 말했거나, 끝내지 않은 관계를 끝났다고 들리게 했다면 문장이 남아 있었다.'),grounded(22,'실제로 남아 있는 설명은 사진과 섞어 지우지 않고, 고쳐 말하거나 아직 못 하겠다고 미룬 사실 그대로 두었다.','나는 불편하다고 사진을 지우지 않았다. 사진은 실제로 좋은 시간이었고, 다른 설명은 내가 한 것이었다.'),mono(22,'고쳐 말할 수 있었다.'),mono(22,'지금은 못 하겠다고 미룰 수도 있었다.'),mono(22,'하지만 미룬 뒤에 상대가 어떻게 알아들었을지까지 내 편한 쪽으로 정할 수는 없었다.'));
  else if(contact)steps.push(grounded(22,'숨긴 말은 없었다. 나는 오늘 하은의 웃음을 떠올리고 내일 어떤 대화를 할지 생각했다.','숨긴 말이 없는 경로에서는 이런 죄책감을 만들지 않았다. 나는 오늘 하은의 웃음을 떠올리고, 내일 어떤 대화를 할지 생각했다. 서로를 계속 만나고 싶다는 마음이 있다고 미래까지 전부 같은 것은 아닐 수 있었다.'),mono(22,'그 다른 점도 듣고 싶었다.'));
...
function scene23(chapter){
  ...
  if(chapter.facts.souvenirPurchase)steps.push(grounded(23,'기념품을 내가 쓰고 싶은 곳에 두었다.','기념품을 샀다면 내가 쓰고 싶은 곳에 두었다. 안 샀다면 빈자리를 채우려고 다른 것을 사지는 않았다.'));
  ...
  else steps.push(grounded(23,'오늘은 연락을 쉬기로 했고 조용한 화면도 편안했다.','오늘 연락을 쉬기로 했다면 조용한 화면이 약속을 어긴 것이 아니었다.'));
