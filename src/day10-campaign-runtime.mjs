...
function rhythmReaction(id){
  ...
  return [d("나","45분, 10분 휴식, 다시 45분. 점심 전 두 블록을 고정하겠습니다."),d("팀장","남는 시간은 연장이 아니라 회복과 기록에 씁니다."),n("세 시간은 빈틈없이 채울 목표가 아니라 멈출 시각이 보이는 틀이 되었다.")];
}
function lunchReaction(id){
  if(id==="work10_lunch_one_question_each")return [d("나","서진 씨에게는 현재 판단 기준 하나, 민호에게는 팀이 바뀐 점 하나만 물을게요."),d("민호","한 사람 말로 회사 전체를 복원하지 않는 방식, 찬성.","smile"),n("두 답은 서로의 영역을 침범하지 않는 현재 정보로 남았다.")];
  if(id==="work10_lunch_quiet_recovery")return [d("나","오늘 점심은 질문 없이 먹고 약을 챙긴 뒤 쉬겠습니다."),d("윤서진","침묵을 불편함이나 평가로 해석하지 않을게요.","calm"),n("같이 먹되 대화를 의무로 만들지 않는 것도 현재 동료 관계의 한 방식이었다.")];
  return [d("나","예전 이야기보다 지금 맡은 일과 최근 바뀐 역할만 듣겠습니다."),d("윤서진","그럼 제 얘기는 현재 프로젝트와 오늘 인계까지만.","smile"),n("점심은 잃어버린 평판을 수집하는 자리가 아니라 현재 팀을 배우는 시간이 되었다.")];
}
...
const segment0=()=>[
  transition("DAY 10 · 세 시간이라는 현재", "home-morning", "girlfriend", "smile"),enter("girlfriend","smile"),
  ...
  transition("SCENE 02 · 자동 확대 없는 출입", "office-day", "office-best-male", "smile"),enter("office-best-male","smile"),
  ...
  transition("SCENE 03 · 오늘의 리듬 계약", "day9-office-project-room-day", "team-lead"),enter("team-lead"),
...
const segment2=state=>[
  ...lunchReaction(state.storyFlags?.day10LunchStrategy),transition("SCENE 06 · 마지막 블록", "day9-office-project-room-day", "female-coworker"),enter("female-coworker"),
  n("점심 뒤 마지막 블록에서는 오전 질문표 한 장만 현재 담당자에게 인계했다."),d("윤서진","추가 자료를 열지 않아도 오늘 범위는 완료됐어요."),
  d("나","완료감 때문에 남은 시간을 채우지는 않겠습니다."),d("팀장","세 시간 종료. 상태와 업무 결과는 따로 평가합니다."),
  transition("SCENE 07 · 귀가 전 세 칸", "office-day", "office-best-male", "smile"),enter("office-best-male","smile"),
...
const segment3=state=>[
  ...debriefReaction(state.storyFlags?.day10DebriefStrategy),transition("SCENE 08 · 세 시간을 하루로 돌려놓기", "home-morning", "girlfriend", "smile"),enter("girlfriend","smile"),
