...
function debriefReaction(id){
  if(id==="work10_debrief_adjust_one_block")return [d("나","두 번째 블록의 피로만 높았습니다. 다음에는 그 구간을 30분으로 줄이겠습니다."),speakerLine("S08_HOME_RETURN","girlfriend","하은","하루 전체를 실패로 만들지 않고 바꿀 한 칸만 찾은 거네.","smile"),n("조정은 후퇴가 아니라 실제 기록에 따른 다음 실험으로 저장됐다.")];
  if(id==="work10_debrief_separate_scores")return [d("나","업무 결과, 회복 상태, 동료 관계를 각각 남길게. 하나가 다른 둘을 대신하지 않게."),speakerLine("S08_HOME_RETURN","girlfriend","하은","잘한 일 때문에 아픈 걸 숨기거나, 피곤해서 사람 관계까지 나빴다고 하지는 않는 거지.","calm"),n("세 개의 점수는 같은 하루를 서로 다른 사실로 보존했다.")];
  return [d("나","오늘 세 시간 리듬은 작동했어. 다음 방문에도 범위를 늘리지 않고 유지할게."),speakerLine("S08_HOME_RETURN","girlfriend","하은","익숙해졌다는 이유로 자동 연장하지 않는 것까지 포함해서.","smile"),n("성공한 일정은 곧바로 더 큰 부담이 아니라 반복 가능한 현재 기준이 되었다.")];
}
