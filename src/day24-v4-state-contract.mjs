...
function newMeetingEligible(chapter){const direction=chapter.facts.contactDirection;return relationshipEnded(chapter)&&chapter.facts.contactCleanup==='RELATIONSHIP_ENDED'&&['LISTEN_TODAY','SHARE_SELF','PERSONAL_INTEREST'].includes(direction);}
function reduceContact(chapter,response){if(chapter.phase!=='contact_resolution'||response?.type!=='otherContactResponse'||!['ACKNOWLEDGED','ACCEPT_MEETING','DECLINE','THINK'].includes(response.outcome))throw new Error('DAY24_INVALID_CONTACT_RESOLUTION');if(response.outcome==='ACCEPT_MEETING'&&!newMeetingEligible(chapter))throw new Error('DAY24_NEW_MEETING_NOT_ELIGIBLE');addResolution(chapter,'CONTACT',{recipient:chapter.facts.contactRecipient,outcome:response.outcome});chapter.facts.otherContactResponse=response.outcome;chapter.facts.newMeetingAccepted=response.outcome==='ACCEPT_MEETING';afterContact(chapter);}
