...
export const EXTORTION_ENCOUNTER_CHANCE = 0.2;
...
const JAEMIN_LOCATIONS = new Set(["prime-gym","boxing-studio","protein-cafe","running-park","climbing-lab"]);
const EXTORTION_LOCATIONS = new Set(["jamsil-station","myeongdong-station"]);
...
export const JAEMIN_QUIZZES = Object.freeze({
  ...
  "running-park":Object.freeze([
    Object.freeze({id:"running-warmup",question:"러닝을 시작하기 전 가장 적절한 준비는?",choices:Object.freeze(["가벼운 걷기와 동적 준비 운동","차가운 상태에서 바로 전력 질주","정적 스트레칭만 오래 하기"]),answer:0,explanation:"걷기와 가벼운 조깅, 동적 움직임으로 체온을 올린 뒤 속도를 높이는 편이 안전해."}),
    Object.freeze({id:"running-talk-test",question:"편안한 유산소 러닝 강도를 확인하는 간단한 방법은?",choices:Object.freeze(["항상 숨이 막힐 때까지 달리기","짧은 문장으로 대화가 가능한지 확인하기","심박수를 전혀 확인하지 않기"]),answer:1,explanation:"대화가 가능한 정도의 강도는 무리하지 않는 유산소 러닝을 판단하는 실용적인 기준이야."}),
    Object.freeze({id:"running-stride",question:"러닝 중 발이 몸보다 지나치게 앞에 닿아 충격이 커진다면 어떻게 해야 할까?",choices:Object.freeze(["보폭을 더 크게 벌린다","상체를 뒤로 젖힌다","보폭을 조금 줄이고 발이 몸 가까이에 닿게 한다"]),answer:2,explanation:"과도한 오버스트라이드는 제동 충격을 키울 수 있어. 자연스럽게 보폭을 줄이고 리듬을 유지해."}),
    Object.freeze({id:"running-hydration",question:"더운 날 러닝할 때 올바른 방법은?",choices:Object.freeze(["기온에 맞춰 강도를 낮추고 수분을 챙긴다","평소보다 두꺼운 옷을 입고 땀을 참는다","갈증이 심해도 끝날 때까지 물을 마시지 않는다"]),answer:0,explanation:"고온에서는 속도보다 체온 관리가 우선이야. 수분을 준비하고 어지러우면 즉시 멈춰야 해."}),
    Object.freeze({id:"running-progression",question:"러닝 거리와 시간을 늘리는 가장 안전한 원칙은?",choices:Object.freeze(["매번 두 배씩 늘린다","몸 상태를 보며 한 번에 한 요소만 점진적으로 늘린다","통증이 있어도 같은 강도를 유지한다"]),answer:1,explanation:"거리·시간·속도를 한꺼번에 올리지 말고 회복 상태를 확인하며 점진적으로 적응해야 해."})
  ]),
  "climbing-lab":Object.freeze([
    Object.freeze({id:"climbing-legs",question:"초보자가 클라이밍에서 팔의 피로를 줄이는 기본 방법은?",choices:Object.freeze(["팔 힘만으로 몸을 당긴다","다리를 펴며 발로 밀고 엉덩이를 벽 가까이 둔다","발을 쓰지 않고 매달린다"]),answer:1,explanation:"큰 다리 근육으로 몸을 밀고 무게중심을 벽 가까이 두면 팔에 쏠리는 부담을 줄일 수 있어."}),
    Object.freeze({id:"climbing-route-read",question:"루트를 오르기 전에 홀드를 살펴보는 이유는?",choices:Object.freeze(["손에 닿는 홀드를 무작정 잡기 위해","다른 사람보다 빨리 출발하기 위해","이동 순서와 쉴 지점을 미리 계획하기 위해"]),answer:2,explanation:"출발 전에 손·발 홀드와 어려운 구간을 읽으면 불필요한 움직임과 힘 낭비를 줄일 수 있어."}),
    Object.freeze({id:"climbing-safety-check",question:"로프 클라이밍을 시작하기 전에 가장 중요한 것은?",choices:Object.freeze(["파트너와 하네스·매듭·빌레이 장비를 서로 확인하기","확인 없이 바로 벽에 오르기","로프를 느슨하게 바닥에 두기"]),answer:0,explanation:"등반자와 빌레이어가 하네스, 매듭, 장비 연결을 서로 확인하는 파트너 체크가 기본이야."}),
    Object.freeze({id:"climbing-fall",question:"볼더링에서 떨어질 때 안전한 방법은?",choices:Object.freeze(["팔을 곧게 뻗어 바닥을 짚는다","착지 공간을 확인하고 무릎을 굽혀 충격을 분산한다","벽에서 최대한 멀리 몸을 던진다"]),answer:1,explanation:"매트와 착지 공간을 먼저 확인하고, 착지 때 관절을 잠그지 말고 충격을 분산해야 해."}),
    Object.freeze({id:"climbing-grip",question:"홀드를 잡을 때 손가락 부상을 줄이는 습관은?",choices:Object.freeze(["항상 가장 강한 크림프만 사용한다","통증이 있어도 계속 반복한다","필요 이상으로 꽉 쥐지 않고 열린 손과 휴식을 활용한다"]),answer:2,explanation:"과도한 그립과 반복되는 강한 크림프는 손가락 부담을 키워. 가능한 편한 그립과 충분한 휴식을 사용해."})
  ])
});
...
export function createWorldEncounterRoutes(){return {version:1,minjun:{step:0,completed:false},jaemin:{answered:{"prime-gym":[],"boxing-studio":[],"protein-cafe":[],"running-park":[],"climbing-lab":[]},correctCount:0,helpOffered:false},junho:{seen:[]}};}
...
export function resolveRepeatWorldEncounter(state,encounter,choiceId){const choice=encounter?.choices?.find(item=>item.id===choiceId),anonymous=encounter?.routeType==="extortion",npc=(state?.npcs??[]).find(item=>item.id===encounter?.npcId)??(anonymous?{id:"anonymous-extortionist",instanceId:"anonymous-extortionist",name:"불특정 인원",active:true}:null);if(!choice||!npc)return null;npc.active=true;const applied={};for(const [key,amount] of Object.entries(choice.npcEffects??{})){const before=Number(npc[key])||0;npc[key]=Math.max(0,Math.min(100,before+(Number(amount)||0)));applied[key]=npc[key]-before;}const playerEffects=applyWorldPlayerEffects(state,choice.playerEffects);let moneyLoss=0;if(choice.cashLossRate){moneyLoss=Math.min(Number(state.money)||0,Math.round((Number(state.money)||0)*Number(choice.cashLossRate)));state.money=Math.max(0,(Number(state.money)||0)-moneyLoss);state.economyLedger??=[];state.economyLedger.push({day:state.day,category:"extortion",label:`${encounter.locationId==="jamsil-station"?"잠실역":"명동역"} 협박 피해`,amount:-moneyLoss});}const routes=ensureRoutes(state);let followUpMessage="";if(encounter.routeType==="minjun"){routes.minjun.step=Math.max(routes.minjun.step,Number(encounter.step)+1);if(encounter.final||routes.minjun.step>=MINJUN_CONCERNS.length){routes.minjun.completed=true;npc.storyState="haeun-boundary-closed";npc.active=false;followUpMessage="민준과의 하은 관련 상담은 끝났습니다. 이후 호수 산책로와 남산 산책길에서 민준은 더 이상 등장하지 않습니다.";}}else if(encounter.routeType==="jaemin"){const answered=routes.jaemin.answered[encounter.locationId]??(routes.jaemin.answered[encounter.locationId]=[]);if(!answered.includes(encounter.questionId))answered.push(encounter.questionId);if(choice.correct)routes.jaemin.correctCount+=1;const relationshipIndex=Math.round(((Number(npc.affection)||0)+(Number(npc.trust)||0))/2);if(!routes.jaemin.helpOffered&&routes.jaemin.correctCount>=15&&relationshipIndex>=38&&npc.affection>=50){routes.jaemin.helpOffered=true;npc.storyState="personal-training-offered";followUpMessage="재민: 문제를 제대로 이해하고 있네. 다음부터 헬스하러 오면 내가 자세도 봐주고 네게 맞는 운동 루틴도 같이 짜 줄게.";}}else if(encounter.routeType==="junho"){if(!routes.junho.seen.includes(encounter.questionId))routes.junho.seen.push(encounter.questionId);}state.worldEncounterHistory??=[];const record={id:encounter.id,day:state.day,minutes:Number(state.nightState?.minutes)||0,locationId:encounter.locationId,npcId:npc.id,choiceId,questionId:encounter.questionId??null,correct:choice.correct??null,effects:applied,playerEffects,moneyLoss};state.worldEncounterHistory.push(record);state.worldEncounterHistory=state.worldEncounterHistory.slice(-120);if(!anonymous){state.npcHistory??=[];state.npcHistory.push({day:state.day,phase:state.phase,actionId:encounter.id,npcId:npc.instanceId,effects:applied});}return {encounter,choice,npc,effects:applied,playerEffects,moneyLoss,record,followUpMessage,routes};}
