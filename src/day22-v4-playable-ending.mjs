...
function reaction17(chapter){const id=last(chapter),steps=[];if(id.endsWith('_choose_photo_tomorrow'))steps.push(mono(22,'사진을 실제로 찍었고 하은도 원하면 내일 각자 마음에 드는 것을 하나 고르기로 했다. 함께 사진이 없다면 풍경이나 기억에 남은 순간을 이야기하기로 했다.'),mono(22,'모든 사진을 검토하는 숙제는 아니었다.'));else if(id.endsWith('_rest_separately'))steps.push(quoted(22,'**하은** “응. 나 도착하면 알려 줄게.”'),quoted(22,'**주인공** “나도.”'),mono(22,'오늘 충분히 함께했으니 지금부터 각자 쉬고 싶다는 말이 자연스러웠다.'));else steps.push(mono(22,'나는 오늘 고마웠던 일이나 좋았던 순간을 말했다.'),quoted(22,'**주인공** “피곤하다고 말해 줘서 좋았어.”'),quoted(22,'**하은** “고마웠다는 말이 좀 신기하다.”'),quoted(22,'**주인공** “네가 참고 있으면 나는 잘된 줄 알았을 것 같아서.”'),mono(22,'하은이 고개를 끄덕였다.'),quoted(22,'**하은** “다음에도 말할게.”'));steps.push(mono(22,'서로 귀가했다. 이 경로에는 낯선 숙소의 아침이 없었다. 내일 각자 집에서 오늘을 떠올릴 것이다.'));return steps;}

function scene24(chapter){const route=chapter.input.route,steps=[scene(24,chapter)];if(route==='BUSAN_TRIP')steps.push(mono(24,'부산의 밤이라면 나는 실제 예약한 공간에 있었다.'));else if(route==='SEOUL_DAY')steps.push(mono(24,'서울의 밤이라면 내 집에 돌아와 있었다.'));else steps.push(mono(24,'떠나지 않은 날이면 익숙한 방에서 오늘의 작은 일을 떠올렸다.'));
  if(route==='NO_TRAVEL')steps.push(mono(24,'혼자 지낸 날이면 나는 내 속도를 조금 더 알았다. 누군가 옆에 없을 때 좋은 것이 있다는 사실이 누군가와 만나고 싶다는 마음을 지우지는 않았다.'));
