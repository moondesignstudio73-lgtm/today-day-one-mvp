import { getGirlfriendCareerAvailability } from "./girlfriend-jobs-data.mjs";
...
export function getInitiatedMessageChance(state) {
  ...
  const careerAvailability=getGirlfriendCareerAvailability(state);
  return clampChance((0.08 + personality.contactImportance * 0.004 + (state.affection >= 700 ? 0.08 : 0) + (state.trust < 350 ? 0.12 : 0) - (contactedToday ? 0.14 : 0))*careerAvailability.multiplier);
}
...
export function maybeGenerateInitiatedMessage(state, random = Math.random) {
  ...
  const careerAvailability=getGirlfriendCareerAvailability(state);
  const careerText=careerAvailability.busy?state.partner.career?.messageLine:null;
  const adaptedText=careerText??(state.partner.heroineId==="yuna"?(state.trust<350?"혹시 내가 뭐 잘못했어? 아니면 바쁜 거야? 답장 천천히 해도 돼.":state.fatigue>=70?"오늘 완전 피곤해 보였어. 밥 먹고 일찍 자! 진짜로.":state.affection>=700?"수업 중에 갑자기 네 생각났어 ㅋㅋ 끝나고 잠깐 볼래?":personality.contactImportance>=65?"나 학원 끝났어! 지금 뭐 해?":state.partner.messageVoice?.greeting??"오늘 학교 끝나고 잠깐 볼래?"):text);
