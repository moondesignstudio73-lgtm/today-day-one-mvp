...
export function getDay23V4Options(chapter){
  if(['ending','photo_resolution','souvenir_resolution','relationship_resolution','conversation_resolution','meeting_resolution'].includes(chapter?.phase))return [];
  ...
  if(number===7&&!chapter.input.landscapePhotoExists&&!chapter.input.sharedPhotoKept)result=result.filter(option=>!option.id.endsWith('_one_photo'));
...
function reduceMain(chapter,id){const phase=chapter.phase,number=mainPhases[phase],variant=phase==='self_intent'?'NO_PENDING_CONTACT':'MAIN',suffixes=phase==='self_intent'?selfIntentSuffixes:mainSuffixes[number],index=indexFor(id,suffixes),facts=chapter.facts;addChoice(chapter,number,variant,id);
  ...
  if(number===9){facts.relationshipIntent=['KEEP_MEETING','DISCUSS_MORE','HOME_COMFORT'][index];chapter.phase='relationship_resolution';return;}
  if(number===10){facts.farewellPlan=['ARRIVAL_LINE','EVENING_CALL','REST_LATER'][index];chapter.phase='home_arrival';return;}
