...
const transition=(label,backgroundId,characterId="girlfriend",expressionId="calm")=>({type:"transition",style:"crossfade",label,backgroundId,characterId,characterAssetUrl:characterId==="girlfriend"?STORY_OUTFIT_ASSETS.day8:undefined,expressionId,poseId:"standing",bgmId:"daily"});
...
const segment0=()=>[
  transition("DAY 12 · 현재 계정의 주인", "home-morning","girlfriend","calm"),enter("girlfriend","calm"),n("식탁에는 DAY 11에 만든 확인 목록, 최근 공과금 세 장, 봉인된 계정 안내서와 임시 결제 영수증이 놓여 있었다."),
  d("하은","잔액을 알게 돼도 오늘은 돈을 움직이지 않는 날로 하자."),d("나","명의, 공식 접근, 현재 생활비까지만. 투자와 과거 소비는 닫아 두고."),
  transition("SCENE 02 · 공식 출처만 남기기", "day2-home-entry","girlfriend","calm"),enter("girlfriend","calm"),n("휴대폰의 검색 결과 대신 명세서에 적힌 공식 앱 주소와 문의 번호를 종이에 옮겼다."),d("하은","내가 기억하는 비밀번호는 확인된 출처가 아니야."),d("나","복구도 공식 본인 확인으로만 진행할게."),
  transition("SCENE 03 · 첫 확인 범위", "home-morning","girlfriend","smile"),enter("girlfriend","smile"),d("하은","어디서 시작하든 확인과 판단을 같은 버튼으로 누르지는 말자."),choice(DAY12_VERIFY_CHOICES)
];
const segment1=state=>[
  ...verifyReaction(state.storyFlags?.day12VerifyStrategy),transition("SCENE 04 · 읽기 전용 화면", "day2-home-entry","girlfriend","calm"),enter("girlfriend","calm"),n("본인 확인 뒤 열린 화면에는 현재 잔액과 이번 달 명세 요약이 보였지만 송금·투자 기능은 별도 잠금 상태였다."),
  d("나","금액은 현재 사실이지만 사용 가능 예산이라는 결론은 아니야."),d("하은","보류된 비용과 소유권이 확인되지 않은 항목부터 빼야겠네."),
  transition("SCENE 05 · 생활비의 소유권", "neighborhood-cafe-day","girlfriend","calm"),enter("girlfriend","calm"),n("둘은 공식 명세와 영수증을 들고 집 근처 카페에서 현재 생활비 항목만 분류했다."),
  d("하은","같이 쓴 전기와 인터넷도 누가 내기로 했는지는 내가 대신 정할 수 없어."),d("나","관계보다 출처와 현재 합의로 분류하자."),choice(DAY12_EXPENSE_CHOICES)
];
const segment2=state=>[
  ...expenseReaction(state.storyFlags?.day12ExpenseStrategy),transition("SCENE 06 · 잔액과 선택 사이", "home-morning","girlfriend","calm"),enter("girlfriend","calm"),n("확인된 잔액 옆에는 사용할 수 있는 돈, 보류할 돈, 아직 소유권을 확인할 돈이 서로 다른 색으로 표시됐다."),
  d("하은","숫자가 보이니까 안심되면서도 뭔가 해야 할 것 같은 기분이 들어."),d("나","그 기분도 판단 근거가 되지는 않게 기록만 할게."),
  transition("SCENE 07 · 어디까지 열 것인가", "day2-home-entry","girlfriend","smile"),enter("girlfriend","smile"),n("기본 계정 접근이 복구되자 앱은 저축과 투자 메뉴도 함께 활성화할지 물었다."),
  d("하은","확인 기능과 자산 판단을 한꺼번에 열지 않아도 돼."),d("나","오늘 허용할 금융 범위를 따로 정하자."),choice(DAY12_ACCESS_CHOICES)
];
const segment3=state=>[
  ...accessReaction(state.storyFlags?.day12AccessStrategy),transition("SCENE 08 · 현재 생활비 장부", "home-morning","girlfriend","smile"),enter("girlfriend","smile"),n("현재 잔액과 공식 명세, 확인된 고정 생활비가 읽기 전용 장부에 저장됐다. 송금·투자·공동 자산 메뉴는 계속 잠겨 있었다."),
...
export function validateLockedDay12Runtime(){const state={storyFlags:{day12VerifyStrategy:"account12_verify_owner_statement",day12ExpenseStrategy:"account12_expense_source_labels",day12AccessStrategy:"account12_access_separate_investment"}};const all=[...segment0(state),...segment1(state),...segment2(state),...segment3(state)];return all.filter(step=>step.type==="transition").length>=8&&all.filter(step=>step.type==="choice").length===3&&all.at(-1)?.type==="sceneEnd";}
