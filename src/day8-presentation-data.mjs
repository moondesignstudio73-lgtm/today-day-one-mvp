...
const READY_BACKGROUNDS=new Set(["home-morning","neighborhood-street-day","neighborhood-cafe-day"]);
const REQUIRED_BACKGROUNDS=new Set(Object.keys(DAY8_REQUIRED_BACKGROUND_ASSETS));
...
export function validateDay8PresentationData(scenes=DAY8_PRESENTATION_SCENES){
  ...
  return values.length===8&&values.every(scene=>{
    ...
    const statusOk=scene.assetStatus==="ready"
      ? backgrounds.every(id=>READY_BACKGROUNDS.has(id))
      : scene.assetStatus==="required"&&backgrounds.every(id=>READY_BACKGROUNDS.has(id)||REQUIRED_BACKGROUNDS.has(id))&&backgrounds.some(id=>REQUIRED_BACKGROUNDS.has(id));
