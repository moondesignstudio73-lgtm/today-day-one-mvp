...
export function runDailyStoryDirector(state,completedDay=state.day) {
  ...
  const eventCandidates=EVENT_DEFINITIONS.map(event=>eventCandidate(state,event,threads,narrativeTension,completedDay)).sort((a,b)=>b.finalProbability-a.finalProbability);const storyCandidates=STORY_SCENES.map(scene=>({id:scene.id,title:scene.title,score:storyScore(scene,threads,nextDay)})).filter(item=>item.score>-500).sort((a,b)=>b.score-a.score);
