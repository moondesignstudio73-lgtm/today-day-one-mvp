export function getDay28V4RuntimeResolution(state,step){const chapter=state?.storyFlags?.day28V4;
  if(step?.type==='day28MeetingCue')return {type:'day28MeetingResponse',target:step.target,outcome:'ACCEPTED',method:chapter?.input?.currentEnergy<30?'CALL':'IN_PERSON'};
