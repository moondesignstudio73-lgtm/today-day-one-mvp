...
export function observePersonality(state, actionTag, random = Math.random) {
  ...
  const current = observations[key] ?? { confidence: 0, samples: 0 };
  current.samples += 1;
  ...
  return { key, confidence: current.confidence, revealed: state.revealedTraits.includes(key) };
}
...
export function getVisibleTraitRows(state) {
  return VISIBLE_TRAITS.map(([key, name]) => {
    ...
    return { key, name, value: revealed ? traitLabel(state.partner.personality[key]) : "???", confidence: observation?.confidence ?? 0, revealed };
  });
}
