...
- [x] 자체 내러티브 QA·정적 계약 검사
- [x] 기존 에셋 감사·연출/오디오 매핑
- [x] 필요한 신규 자산 제작·이미지 QA (`NOT REQUIRED` — 기존 검증 자산으로 충족)
...
DAY 9의 8개 Scene·3단계 전략 선택·DAY 5/8 콜백·중간 저장 복원을 실제 런타임에 구현한다.

### 2026-08-26 DAY 9 기존 에셋·연출·오디오 관문 기록

- 집·낮 거리·사무실 배경과 하은·민호·윤서진·팀장·주니어 스프라이트를 원본 해상도와 투명도로 육안·기술 감사했다.
- 동일 회사의 로비·팀 자리·작은 회의실은 `office-day` 안의 구역을 카메라와 발화자 교대로 분리해 장소 연속성을 보존했다.
- 8개 Scene의 배경·인물 교대·카메라·전환, `daily` BGM과 기존 소품 SFX를 `src/day9-presentation-data.mjs`에 고정했다.
- 기존 자산만으로 잠금 시나리오의 장소·역할·감정 온도를 충족해 신규 자산 제작과 별도 이미지 QA를 `NOT REQUIRED`로 판정했다.
