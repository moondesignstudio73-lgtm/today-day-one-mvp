...
## DAY 17 V4 PUBLIC RELEASE COMPLETE (2026-09-04 KST)

- Notion `DAY 17 — 잘하고 싶은 몸 | SCENARIO V4` 기준 24 Scene·주요 선택 12개와 대면/미대면·유리 초대/미초대 대체 경로를 게임에 연결했다.
- DAY 16 V4 플래그를 입력으로 소비하고 선택·체크포인트·하은 공개·내일 약속·DAY 18 훅을 저장한다. 기존 DAY 17 간략판 저장은 레거시로 보존한다.
- DAY 13~16과 비교해 압축된 대사 밀도, 산책/집 경로의 잘못된 헬스장 화자, 실제 행동과 다른 대면 보고, 겪지 않은 유리 메시지·농담·접촉 가능성을 발견하고 원문 순서·경로 사실에 맞게 수정했다.
- 실제 브라우저에서 대면·유리 초대 경로 12선택과 집 휴식·비대면·유리 미초대 경로 9선택을 각각 완주해 DAY 18 진입을 확인했다. 미초대 경로의 유리 메시지는 0건이고, 390×844 모바일 선택 UI의 수평 오버플로도 0이다.
- V4 종료 뒤 과거 복약 자유행동이 난입하는 HIGH 결함을 레거시 저장에서만 노출되도록 수정했다.
- 집중 6/6, 전체 Node 364/364, 100회 × 30일 시뮬레이션 PASS. 구현 커밋 `3d5e6f7`을 대상 저장소의 `main`·`gh-pages`에 반영했고 Pages run `33782479070`이 같은 SHA로 `completed/success`를 반환했다.
- 공개 사이트에서 `game.js?v=183`, DAY 17 V4 모듈 import와 모듈 HTTP 200을 확인했다. DAY 17 PUBLIC RELEASE는 COMPLETE이며 DAY 18 본문 구현은 시작하지 않았다.
