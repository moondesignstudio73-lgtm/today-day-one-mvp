...
기존 prepare/apply 선택만 사용하는 `하은 도착 가방 QA` 집중 진입점을 추가했다. game224에서 하은 도착 대사를 SKIP 없이 진행해 “의자 맡아 둔 사람이 더 좋아.” 직후 새 CG, 19:00을 확인했다. CG 표시 중 새로고침/이어하기 후 장면 시작에서 같은 CG가 다시 나오고 메뉴 세 선택으로 정상 복귀했다. 콘솔 error/warning 없음, 사용자 저장 복원 완료.

이 집중 픽스처는 아침부터 완주 증거가 아니며, 선택 화면에 직전 하은 대사가 남는 기존 표시 방식까지 새 행동으로 집계하지 않는다. 가방 CG 실제 표시/저장 재개는 PASS. 다음은 미구현인 주인공 메뉴 밀기다.
