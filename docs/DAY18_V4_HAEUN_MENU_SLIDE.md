...
메뉴 밀기 CG가 표시되는 동안 새로고침했다. 이어하기 뒤 장면 시작에서 다시 진행해 가방 내려놓기 → 메뉴 밀기 → 메뉴 세 선택의 순서를 비-SKIP으로 확인했다. 19:00 유지, 콘솔 error/warning 없음, 사용자 저장 복원 완료. 메뉴 밀기 저장 재개는 PASS이며 집중 픽스처를 아침부터 완주 증거로 확대하지 않는다.

다음은 SOLO 맞은편 가방 이동 행동이다. 같은 '가방'이라도 주인공 소유이고 다른 손님 때문에 옮긴 행동이므로 하은 자산/flag를 재사용하지 않는다.
