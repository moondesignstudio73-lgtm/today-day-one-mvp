...
재킷을 의자에 거는 CG가 표시되는 동안 새로고침했다. 이어하기 후 기존 계약대로 장면 시작에서 다시 진행해 메뉴 닫힘/펼침 → 재킷 입은 유리 도착 → 재킷 CG → 기본 블라우스 인물의 순서를 확인했다. 같은 실행에서 SCENE04 각자 메뉴를 선택한 뒤에도 기본 유리 인물, 숨겨진 CG, 새 원문 독백을 확인했다. 19:00 유지, 콘솔 error/warning 없음, 사용자 저장 복원 완료.

관찰 중 숨겨진 CG img의 src 속성은 직전 파일을 유지했지만 `isVisible=false`였고 대화 화면에는 노출되지 않았다. 이를 잔류 화면으로 잘못 집계하지 않는다. 겉옷 저장 재개/후속 의상 QA는 PASS다. 다음은 하은 도착 가방/메뉴 행동이다.
