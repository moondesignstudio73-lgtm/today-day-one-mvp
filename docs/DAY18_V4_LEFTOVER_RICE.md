...
SCENE03 이후 메뉴·코트·가방 등 남은 행동 연출과 알람 입력/소리를 구현해야 한다. 이후 동일 버전 4경로, 모바일, DAY15~17 연속성 감사를 모두 통과하기 전까지 DAY18은 COMPLETE로 승격하지 않는다.

## 02:27 후속 실제 QA

game219에서 유리 약속 경로를 아침부터 SKIP 없이 진행했다. 약속 유지 → 낮 12:30 미전송 `저녁` → 단어 지우기 → 빈 입력란 → 답장 선택의 세 버튼을 확인했다. 종료된 초안이 화면에 남지 않고 `문자 · 하은` 스타일이 복원됐다.

“오늘 저녁은 혼자 먹을 것 같아.”를 선택한 뒤 하은의 “따뜻한 거 먹어.” → “하은은 내 거짓말을 알아맞히지 못했다. 그게 안심되면서, 안심하는 내가 불편했다.” → 19:00 SCENE03 전환을 관찰했다. 각 입력 사이 CG 표시 여부를 조회했고 이 구간은 모두 false였다. 진짜 SOLO용 남은 밥 CG가 거짓말 분기에 섞이지 않았다. 콘솔 error/warning은 없었고 같은 탭에서 테스트 전 저장을 복원했다. 자연 독서시간이나 DAY18 전체 완주 증거로 사용하지 않는다.
