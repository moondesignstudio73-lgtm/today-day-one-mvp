...
- 기존 `day20CurrentSharedMealPending`은 legacy 선택기의 임시 호환 표식이다. DAY20 V4의 사실 원천은 검증된 `day19V4` 스냅샷이다. DAY19 V4 완료 hook은 DAY20 slot을 대면/solo 모두에 열되, 방문 여부는 `tomorrowMeal === ACCEPTED`와 현재 연락 가능 상태를 함께 검사한다.
...
- playable: conflict/ending SCENE19~22/24 PASS. 갈등은 SCENE19→21→24로 닫혀 친밀·숙박을 건너뛴다. C13 숙박 제안은 `haeunStayResolution`에서 멈추며, 거절은 SCENE21, 준비와 잠자리 합의를 포함한 수락만 SCENE22/C14로 간다. 작별 포옹과 다음 초대는 실제 기록 없이 생성하지 않는다.
...
- DAY20 COMPLETE: **아님**.
- 다음 시작점: DAY20 browser fixture를 추가해 데스크톱과 389×844에서 face/short/solo/conflict/stay/leave를 SKIP 없이 완주하고 콘솔·레이아웃·Story/Free 배타성을 증거화한다.
