...
3. ~~DAY24~26 실제 이력을 동결하는 `day27-notion-v4/1` replay-locked 상태 계약과 legacy 진입 분리를 구현한다.~~ 완료. 실제 수신자·관계 상태 발언·정정 상태·하은 관계 결과·공개 선행 발언·DAY27 청취 대상·연락/거리 요청과 현재 자원을 input seal로 고정한다. 대화 성립, 거짓말 수신자의 대화 지속, 하은의 청취, 관계 결과, 지훈 가능 여부, 다음 약속은 각각 독립 현재 응답으로 기록하며 비대화 경로는 듣지 못한 응답을 생성하지 않는다.
4. ~~SCENE01~09/C1~7 대화 시작·실제 수신자 관계 진실 경로를 exact-source ref로 구현한다.~~ 완료. 아침 C1 뒤 현재 대화 응답 전에는 상대를 등장시키지 않으며, 대화가 없으면 SCENE23으로 교차한다. 대화 수락 뒤 실제 봉인 수신자의 이름·대사만 SCENE03에 투영하고 C3의 네 선택, 수신자의 대화 지속/종료, C4 사과, C5 하은 고지와 하은의 현재 청취 응답, C6~7 및 독립 관계 응답을 분리했다. 수신자가 끝내면 SCENE05~09와 하은의 반응을 만들지 않는다.
...
SCENE18~24/C13~16에서 지훈의 현재 가능 응답, 저녁, 정직·관계 지속·연락 허용 밤, 이별 밤, 수신자별 정정과 비대화 대체 C3~8, DAY28 handoff를 exact-source projection으로 구현한다.
