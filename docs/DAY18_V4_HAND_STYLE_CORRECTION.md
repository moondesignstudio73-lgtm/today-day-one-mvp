...
## 2026-09-07 v10 정식 호스트 캐시 경계 교정

- 사용자 재신고 화면은 v9 이전의 실사형 전경 손 컷과 동일하지만, 로컬 및 정식 배포의 v1/v9 바이트는 이미 전경 손이 없는 승인본이었다. 같은 픽셀을 다시 생성하지 않고 v9 승인 비트맵을 v10 새 파일명으로 재발행했다.
- 런타임과 공통 렌더러는 v10 및 `hand-review-20260907-2`만 요청한다. v1~v9는 v10과 바이트 동일한 안전 대체본이며, 최상위 진입점은 `game.js?v=292`, DAY18 bridge/playable은 v64다.
- 정식 배포 주소와 소스 저장소의 오래된 Pages 주소를 혼용하지 않도록 canonical 링크와 `today-day-one-build=20260907-hand-review-2`를 진입점에 기록했다. 이후 신고는 호스트와 빌드 리비전을 함께 확인한다.
