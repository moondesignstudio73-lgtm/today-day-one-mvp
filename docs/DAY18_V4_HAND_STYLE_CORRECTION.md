...
- 유리 물 장면은 v5, 나머지 교체 자산은 위 v2를 참조한다.
- 반려된 유리 물 v1·v2·v3·v4와 나머지 v1 이름은 실행 소스에 다시 들어갈 수 없다.
- 자동 테스트는 모든 반려 파일명이 생성된 DAY18 세그먼트 어디에도 나타나지 않는지, 승인된 v4/v2 자산이 실제로 존재하는지 함께 검사한다.
...
- 배포 이력 v1~v4와 승인본 v5의 SHA-256은 모두 `3CC2105D5AC12176E0067BEAE44EB01F0E60B71F696C243E44164903D5FFBDD1`로 동일하다.
